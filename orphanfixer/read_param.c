...
void set_simulation_params(struct params_data *params)
{
  ...
  for(int i=PARAMS.MIN_SNAPSHOT_NUM;i<=PARAMS.MAX_SNAPSHOT_NUM;i++) {
	  params->Age[i] = get_age(REDSHIFT[i]);/* Age of the Universe in GYR corresponding to the redshifts */
...
void sanity_check_params(struct params_data *params)
{
  ...
  if((params->MAX_SNAPSHOT_NUM <= params->MIN_SNAPSHOT_NUM) || params->MAX_SNAPSHOT_NUM < 0  || params->MIN_SNAPSHOT_NUM < 0)
	{
	  fprintf(stderr,"ERROR: [min,max] snapshot numbers = [%d,%d] are not well-behaved ...exiting\n",
