...
int64 get_best_groupnum_wids(int64 *sourceIds, int64 Nids,
                             struct group_data *dest, int64 destNgroups,
                             int flag, double *rank, short *DestPartIds,
                             int64 *DestGroupIds, int64 *DestGroupLoc,
                             int64 DestMaxPartId);
...
short load_found_progenitors(struct node_data *tree[], int64 *Ngroups,
  ...
  struct node_data *thisnode = NULL, *BaseNode = NULL, *prevnode = NULL,
                   *tmp_node = NULL;
  ...
  while (1) {
    if (fgets(str_line, MAXLINESIZE, fp) != NULL) {
      if (str_line[0] != comment) {
        sscanf(str_line,
               "%hd  %" STR_FMT "  %" STR_FMT "  %" STR_FMT "  %hd  %" STR_FMT
               "  \n",
               &snapshot, &groupnum, &groupid, &groupN, &prevsnap, &prevnum);

        if (snapshot < PARAMS.MIN_SNAPSHOT_NUM ||
        ...
        if (prevsnap < PARAMS.MIN_SNAPSHOT_NUM ||
            prevsnap > PARAMS.MAX_SNAPSHOT_NUM) {
          fprintf(stderr,
        ...
        if (groupnum > Ngroups[snapshot] || prevnum > Ngroups[prevsnap]) {
          fprintf(stderr, "Inside load_found_progenitors: This should not have "
                          "happened\n ");
          fprintf(stderr,
                  "snapshot = %hd   groupnum = %" STR_FMT
                  " prevsnap = %hd prevnum = %" STR_FMT " \n",
                  snapshot, groupnum, prevsnap, prevnum);
          fprintf(stderr,
                  "Ngroups[%hd] = %" STR_FMT " Ngroups[%hd] = %" STR_FMT " \n",
                  snapshot, Ngroups[snapshot], prevsnap, Ngroups[prevsnap]);
          fprintf(stderr, "exiting \n\n");
          exit(EXIT_FAILURE);
        }
...
int64 get_best_groupnum_wids(int64 *sourceIds, int64 Nids,
      ...
      if (DestPartIds[index] == 1) {
        ...
        if (flag == 1) {
          if (PARAMS.MAX_RANK_LOC <= 0 ||
              (PARAMS.MAX_RANK_LOC > 0 && i < PARAMS.MAX_RANK_LOC))
            DestRanks[grp_index] +=
                compute_rank(i); /* matching based on rank */

          if (PARAMS.MAX_RANK_LOC <= 0 ||
              (PARAMS.MAX_RANK_LOC > 0 &&
               DestGroupLoc[index] < PARAMS.MAX_RANK_LOC))
  ...
  *rank = max_rank;
  if (max_rank > 0.0)
    return dest[max_ranknum]
        .nodeloc; /* dest might not start at 0 -- use nodeloc to return the
                     actual group number */
...
void fillprogenitors(struct node_data *tree[], int64 *Ngroups) {
  ...
  char fname[MAXLEN];
  FILE *fp = NULL;
  struct node_data *BaseNode = NULL, *thisnode = NULL, *NewBaseNode = NULL,
                   *checknode = NULL;
  ...
  */
  ...
  short snapshot = 0;
  for (short isnapshot = startsnapshot; isnapshot >= PARAMS.MIN_SNAPSHOT_NUM;
       isnapshot--) {
    fprintf(
        stderr,
    ...
    if (Ngroups[isnapshot] > 0) {
      for (short incr = 0; incr <= (PARAMS.MAX_DECR_GROUPS + 2); incr++) {
        ...
        if (snapshot >= PARAMS.MIN_SNAPSHOT_NUM &&
            allgroups[snapshot] == NULL && Ngroups[snapshot] > 0) {
          fprintf(stderr,
          ...
          for (int64 i = 0; i < Ngroups[snapshot]; i++) {
            assert(group0[i].nodeloc == i &&
                   "nodeloc has been correctly initialized");
          ...
          if (DestMaxPartId[snapshot] < max_part_id) {
            fprintf(stderr,
                    "Replacing DestMaxPartId[%d] = %" STR_ID_FMT
                    " with %" STR_ID_FMT " NUMPART = %" STR_FMT "\n",
        ...
        if (snapshot >= PARAMS.MIN_SNAPSHOT_NUM &&
            DestPartIds[snapshot] == NULL && incr >= 2) {
          group0 = allgroups[snapshot];
          DestPartIds[snapshot] = my_calloc(sizeof(*DestPartIds[snapshot]),
                                            DestMaxPartId[snapshot]);
          DestGroupIds[snapshot] = my_malloc(sizeof(*DestGroupIds[snapshot]),
                                             DestMaxPartId[snapshot]);
          DestGroupLoc[snapshot] = my_calloc(sizeof(*DestGroupLoc[snapshot]),
                                             DestMaxPartId[snapshot]);
      ...
      for (int64 igroup = startgroup; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (thisnode->BigChild == NULL && thisnode->isFof == 0 &&
          ...
          */
          ...
          for (int skip = 2; skip <= PARAMS.MAX_DECR_GROUPS; skip++) {
            fprintf(
                stderr,
                "Inside fillprogenitors: missing haloid = %" STR_FMT
                " with groupnum = %" STR_FMT " at snapshot = %d skip = %d \n",
                thisnode->haloid, thisnode->nodeloc, thisnode->snapshot, skip);
            searchsnapshot = thisnode->snapshot - skip;
            if (searchsnapshot < PARAMS.MIN_SNAPSHOT_NUM ||
                Ngroups[searchsnapshot] == 0)
              continue;

            group1 = allgroups[searchsnapshot];
            searchnodenum = get_best_groupnum_wids(
                TrackIds, Nids, group1, Ngroups[searchsnapshot],
                MATCH_WITH_RANK, &rank, DestPartIds[searchsnapshot],
                DestGroupIds[searchsnapshot], DestGroupLoc[searchsnapshot],
                DestMaxPartId[searchsnapshot]);

            if (searchnodenum != -1) {
              NewBaseNode = tree[searchsnapshot];
              checknode = &NewBaseNode[searchnodenum];
              assert(searchnodenum < Ngroups[searchsnapshot] &&
                     "Possible match must be valid");
              assert(thisnode->nodeloc < Ngroups[thisnode->snapshot] &&
                     "Candidate node number must be valid");
              ncommon = get_ncommon(&group0[thisnode->nodeloc],
                                    &group1[searchnodenum]);
              fprintf(stderr,
                      "found a possible match:  nodenum = %" STR_FMT
                      " at snapshot = %d. haloid = %" STR_FMT
                      " with ncommon = %" STR_FMT " out of Npart = %" STR_FMT
                      "\n",
                      searchnodenum, searchsnapshot, checknode->haloid, ncommon,
                      group1[searchnodenum].N);
              ...
              if (checknode->Parent == NULL ||
                  (checknode->Parent->BigChild != checknode &&
                   (double)ncommon / (double)group0[thisnode->nodeloc].N >
                       PARAMS.MIN_FCOMMON_THRESH)) {

                if (checknode->Parent != NULL &&
                    checknode->Parent->Nchild > 1) {
                ...
                fprintf(stderr,
                        ...
                        "groupnumber = %" STR_FMT " \n",
                        thisnode->nodeloc, thisnode->snapshot,
                        checknode->snapshot, checknode->nodeloc);
                fprintf(fp,
                        "%6d    %12" STR_FMT "    %14" STR_FMT
                        "     %16" STR_FMT "    %10d   %12" STR_FMT
                        "   %14" STR_FMT "    %14" STR_FMT
                        "      %16.4lf  %16.4lf\n",
                        thisnode->snapshot, thisnode->nodeloc, thisnode->haloid,
                        group0[thisnode->nodeloc].N, checknode->snapshot,
                        checknode->nodeloc, group1[checknode->nodeloc].N,
                        ncommon, rank, max_rank);
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM; isnapshot <= startsnapshot;
       isnapshot++) {
  ...
  if (flag == 1) {
    ...
    print_time(t_sectionstart, t_sectionend,
               "assign_haloid (after filling in progenitors)");
  }
}
