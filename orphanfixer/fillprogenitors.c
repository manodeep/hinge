...
{
  //There is a small bug in the code where the memory for [startsnapshot] does not get freed until the end.
  int flag;
  int64 searchnodenum;
  ...
  */
  ...
  if(PARAMS.LOAD_PARTIAL_FOUND_PROGENITORS==1) {
    ...
    fp = my_fopen(fname,"w");
    fprintf(fp,"#######################################################################################################################################################################\n");
    fprintf(fp,"# Snapshot      Groupnum            GroupId              GroupN       PrevSnap        PrevNum          PrevN            Ncommon               Rank           Max_rank  \n");
    ...
    if(Ngroups[isnapshot] > 0) {
      for(short incr=0;incr <= (PARAMS.MAX_DECR_GROUPS+2);incr++) {
				...
				if (snapshot >= PARAMS.MIN_SNAPSHOT_NUM && allgroups[snapshot] == NULL && Ngroups[snapshot] > 0) {
					fprintf(stderr,"\nIn fillprogenitor: Loading groups for snapshot  %4d, Ngroups = %"STR_FMT"....\n",snapshot,Ngroups[snapshot]);
					group0 = allocate_group(Ngroups[snapshot]);
					loadgroups(snapshot,group0);
					fprintf(stderr,"\nIn fillprogenitor: Loading groups for snapshot  %4d, Ngroups = %"STR_FMT"....done\n",snapshot,Ngroups[snapshot]);
					...
					for(int64 i=0;i<Ngroups[snapshot];i++) {
						...
						for(int64 j=0;j<group0[i].N;j++) {
							...
							if(this_id > max_part_id) max_part_id = this_id;
						}
					}
					max_part_id++;
					...
					//new scope tmp and s will disappear outside the closing braces
					{
						...
						for(int64 i=0;i<Ngroups[snapshot];i++) {
							for(int64 j=0;j<group0[i].N;j++) {
								...
								const id64 this_id = group0[i].id[j];
								assert(this_id < DestMaxPartId[snapshot] && "Particle id must be less than max. particle id - strange things must have happened");
								s[this_id]=1;
					...
					*/
					...
					for(int skip=2;skip<=PARAMS.MAX_DECR_GROUPS;skip++) {
						...
						if(searchnodenum != -1) {
							...
							assert(thisnode->nodeloc < Ngroups[thisnode->snapshot] && "Candidate node number must be valid");
							ncommon = get_ncommon(&group0[thisnode->nodeloc],&group1[searchnodenum]);
							fprintf(stderr,"found a possible match:  nodenum = %"STR_FMT" at snapshot = %d. haloid = %" STR_FMT " with ncommon = %"STR_FMT" out of Npart = %"STR_FMT "\n",
											searchnodenum,searchsnapshot,checknode->haloid,ncommon,group1[searchnodenum].N);
							/* I am taking out the ParentLevel >= 2. As long as the FOF can be re-assigned here, and it is not
