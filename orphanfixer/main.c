...
#undef RETURN_ONLY_FOFS
    ...
    if (Ngroups0 > 0) {
      if (isnapshot < PARAMS.MAX_SNAPSHOT_NUM) {
        ...
        fprintf(stderr,
                "Reading in parents from `%s' Ngroups0 = %" STR_FMT "\n",
                outfname, Ngroups0);
  ...
  if (PARAMS.LOAD_FOUND_PROGENITORS == 1) {
    my_snprintf(outfname, MAXLEN, "%s/found_progenitors.txt",
                PARAMS.OUTPUT_DIR);
    t_sectionstart = time(NULL);
    {
      int64 startgroup = 0;
      load_found_progenitors(
          tree, Ngroups, outfname,
          &startgroup); /* tmp_snapshot is useless in this section */
    }
    assign_haloid(tree, Ngroups);
    t_sectionend = time(NULL);
...
void output_data_for_SUSSING_TREES(struct node_data *tree[], int64 *Ngroups) {
  ...
  my_snprintf(outfname, MAXLEN, "%s/HINGE_Sinha_DATASET-II.txt",
              PARAMS.OUTPUT_DIR);
  ...
  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
    ...
    for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
      ...
      fprintf(fp, "%ld  \t %" STR_FMT " \n", thisnode->SUSSING_haloID,
              thisnode->Nchild);
      if (thisnode->isFof == 0 && thisnode->BigChild == NULL &&
