...
#error  Please select only one of the cosmologies from various WMAP options in the Makefile
  ...
  my_snprintf(snapshotname,MAXLEN,"%s/%s_%03d",PARAMS.SNAPSHOT_DIR,PARAMS.SNAPSHOT_BASE,PARAMS.MAX_SNAPSHOT_NUM);//use this line if only one snapshot is available.
#else
  my_snprintf(snapshotname,MAXLEN,"%s/snapdir_%03d/%s_%03d",PARAMS.SNAPSHOT_DIR,PARAMS.MAX_SNAPSHOT_NUM,PARAMS.SNAPSHOT_BASE,PARAMS.MAX_SNAPSHOT_NUM);//use this line if only one snapshot is available.
#endif
  ...
  for(int i=0;i<6;i++) {
    //needed to make Rvir calculation
...
/*   t_sectionend = time(NULL); */
/*   print_time(t_sectionstart,t_sectionend,"For SUSSING Merger Tree comparison project"); */
/* #endif   */
...
void assign_node(struct group_data *group0,int64 Ngroups0,struct parent_data *parent,struct node_data *node,int isnapshot)
{
  ...
  for(int64 igroup = 0;igroup<Ngroups0;igroup++) {
    ...
    for(int k=0;k<3;k++)
      ...
      /* Make sure that the last fof halo also has the bound fof halo mass assigned. */
...
void readsubhalo_hierarchy_levels(const char *fname,struct group_data *group0)
{
  ...
  while(1) { 
	...
	//The first field in the hierarchy file is of type int64. But since I am suppressing 
	//that field -> I can as well use the %d type [and this way the compiler warning goes away.
...
void output_data_for_SUSSING_TREES(struct node_data *tree[],int64 *Ngroups)
{
  ...
  fprintf(fp,"1\n");
  fprintf(fp,"Halo Interaction Network and Galaxy Evolution - settings used from previous experience with SUBFIND. v1.0, %s \n",date);
  fprintf(fp,"%"PRId64"\n",SumTotal_AllHalos);
...
#else
  fprintf(stderr,"The code was *NOT* compiled to read in velocities from the group files \n");
#endif

#ifdef GET_MEANVEL
  fprintf(stderr,"The code was compiled to read allgroupvels_???.txt files for velocity info\n");
#else
  fprintf(stderr,"The code was *NOT* compiled to read allgroupvels_???.txt files for velocity info\n");
#endif


#ifdef LONGIDS
  fprintf(stderr,"Assumes that Gadget particle ids are 8 bytes (but the total particle number is inside INT_MAX)\n");
#else
  fprintf(stderr,"Assumes that Gadget particle ids are 4 bytes (and the total particle number is inside INT_MAX)\n");
#endif
