...
void output_missing_halo_centres(struct node_data *tree[], int64 *Ngroups) {
  ...
  my_snprintf(snapshot_name, MAXLEN, "%s/%s_%03d", PARAMS.SNAPSHOT_DIR,
              PARAMS.SNAPSHOT_BASE, PARAMS.MAX_SNAPSHOT_NUM);
  ...
  if (check_totnpart != TOTNPART) {
    ...
    fprintf(stderr,
            "Got TOTNPART  = %" PRId64
            " from COSMO struct but snapshot header shows %" PRId64
            " particles\n",
  ...
  for (short isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
       isnapshot > PARAMS.MIN_SNAPSHOT_NUM; isnapshot--) {
    prevsnapshot = isnapshot - 1;
    if (prevsnapshot == PARAMS.MIN_SNAPSHOT_NUM)
      break;
    ...
    if (Ngroups[isnapshot] > 0) {
      ...
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (tmpnode != NULL && (thisnode->snapshot - tmpnode->snapshot) > 1) {
          ...
          /* 				  Nids +=  (group0[igroup].N > MAXPARTLOC) ?
           * MAXPARTLOC:group0[igroup].N; */

          if (PARAMS.MAX_RANK_LOC <= 0) {
            Nids += group0[igroup].N;
          } else {
            Nids += (PARAMS.MAX_RANK_LOC < group0[igroup].N)
                        ? PARAMS.MAX_RANK_LOC
                        : group0[igroup].N;
          }

          // tmpnode exists => I have to track the group to tmpnode->snapshot +
          // 1
          ...
          Track_NumPart_in_Group =
              my_realloc(Track_NumPart_in_Group, sizeof(int64), Nids,
                         "Track_NumPart_in_Group");
          EarliestSnapshot_to_Track =
              my_realloc(EarliestSnapshot_to_Track, sizeof(short), Nids,
                         "EarliestSnapshot_to_Track");

          for (short missing_snapshot = isnapshot - 1;
               missing_snapshot >= tmpnode->snapshot + 1; missing_snapshot--) {
            AllNmissing[missing_snapshot]++;
          }

          TrackHaloIds =
              my_realloc(TrackHaloIds, sizeof(int64), Nmissing, "TrackHaloIds");
      ...
      // new scope
      {
        ...
        for (int64 k = 0; k < Nids; k++) {
          ...
          /* 				fprintf(stderr,"TrackIds[%"STR_FMT"] =
           * %"STR_ID_FMT" TrackNums[%"STR_FMT"] = %"STR_FMT"\n", */
          /* 						k,TrackIds[k],k,TrackNums[k]);
           */
        }
        if (bad > 0) {
          fprintf(stderr,
                  "ERROR: number of particleids less than 0 or greater than "
                  "TOTNPART (%" PRId64 ") = %" STR_FMT " ..exiting\n",
                  TOTNPART, bad);
          exit(EXIT_FAILURE);
        }
      }

      // Now all the particles that need to be tracked have been fit into
      // TrackIds and TrackNums Use the previous snapshot
      for (int i = 0; i < 3; i++) {
      ...
      // sort the missing particles such that particles that need to be tracked
      // the longest appear at the front

#define MULTIPLE_ARRAY_EXCHANGER(type, a, i, j)                                \
  {                                                                            \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(short, EarliestSnapshot_to_Track, i, j);    \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(int64, TrackNums, i, j);                    \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(id64, TrackIds, i, j);                      \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(int64, Track_NumPart_in_Group, i, j);       \
  }

      // There probably are particles that need to be tracked more than one
      // snapshot. Track them through successive snapshots until they are done.
      short snapshot = prevsnapshot;
      while (snapshot >= EarliestSnapshot &&
             snapshot > PARAMS.MIN_SNAPSHOT_NUM && Nids > 0) {
        // sort the array by earliest snapshot
        SGLIB_ARRAY_QUICK_SORT(short, EarliestSnapshot_to_Track, Nids,
        ...
        // new scope -- make sure that the sorting in earliest snapshot works
        // ..(as I think it should -- earliest snapshots in the beg. and later
        // ones
        //  at the end. so the later snapshots/particles willl get done earlier
        //  and then I can merrily realloc and discard those last parts
        {
          int64 bad = 0;
          for (int64 k = npart; k < Nids; k++)
            if (EarliestSnapshot_to_Track[k] <= snapshot) {
              fprintf(stderr,
                      "This does not work. I have earliest snapshot to "
                      "track[%" STR_FMT "] = %d\n",
        ...
        // So the previous check worked - reset Nids to contain the actual
        // number of particles that need to be checked at this snapshot
        // (including earlier snapshots - those will come in the next iteration
        // of the snapshot while loop)
        Nids = npart;
        TrackIds = my_realloc(TrackIds, sizeof(id64), Nids, "TrackIds");
        TrackNums = my_realloc(TrackNums, sizeof(int64), Nids, "TrackNums");
        Track_NumPart_in_Group =
        ...
        SGLIB_ARRAY_QUICK_SORT(id64, TrackIds, Nids, SGLIB_NUMERIC_COMPARATOR,
                               MULTIPLE_ARRAY_EXCHANGER);

        /* 			  fprintf(stderr,"Working on snapshot = %4d Number
         * of missing particles = %12"STR_FMT" earliest snapshot to track to =
         * %4d\n", */
        ...
        while (ifile < nfiles && totnleft > 0) {
          if (nfiles == 1) {
            my_snprintf(snapshot_name, MAXLEN, "%s/%s_%03d",
                        PARAMS.SNAPSHOT_DIR, PARAMS.SNAPSHOT_BASE, snapshot);
          } else {
            my_snprintf(snapshot_name, MAXLEN, "%s/%s_%03d.%d",
                        PARAMS.SNAPSHOT_DIR, PARAMS.SNAPSHOT_BASE, snapshot,
                        ifile);
          }

          fprintf(stderr,
                  "snapshotfile = `%s' Nmissing = %" STR_FMT " Nids = %" STR_FMT
                  "  totnleft = %" STR_FMT " Earliestsnap = %hd \n",
          ...
          for (int k = 0; k < 6; k++) {
            for (int n = 0; n < header1.npart[k]; n++) {
              ...
              if (tmp_id <= 0 || tmp_id > TOTNPART) {
                fprintf(stderr,
                        "ERROR: I have id = %" STR_ID_FMT
                        " but it should be between 0 and %" PRId64 " \n",
                        tmp_id, TOTNPART);
                exit(EXIT_FAILURE);
              }
              ...
              if (location != -1) {
                if (location >= Nids || location < 0) {
                  fprintf(stderr,
                          "ERROR: location is beyond array range location = "
                          "%" STR_FMT " tmp_id = %" STR_ID_FMT
                          " index = %" STR_FMT "\n",
                ...
                if (TrackIds[location] != tmp_id) {
                  fprintf(stderr,
                          "ERROR: Did not get the correct location.. "
                          "TrackIds[%" STR_FMT "] = %" STR_ID_FMT
                          "  tmp_id = %" STR_ID_FMT "\n",
                  ...
                  /* 								  fprintf(stderr,"totnleft
                   * = %"STR_FMT" TrackNums[%"STR_FMT"] = %"STR_FMT" Nfound =
                   * %"STR_FMT"\n", */
                  ...
                  for (int ii = 0; ii < 3; ii++) {
                    SubCM[ii][TrackNums[location]] += periodic(
                        tmp_pos[ii] - zero_part_pos[ii][TrackNums[location]]);
          ...
          fprintf(stderr,
                  "after reading the snapshot: totnleft = %" STR_FMT
                  "  index = %" STR_FMT " nmisses=%" STR_FMT
                  " index-nmisses=%" STR_FMT "\n",
        ...
        // output the data
        my_snprintf(fname, MAXLEN, "%s/ghost_halos_%03d.txt", PARAMS.OUTPUT_DIR,
                    snapshot);
        ...
        for (int64 i = 0; i < Nmissing; i++)
          if (Nfound_in_Group[i] > 0)
            fprintf(fp,
                    ...
                    snapshot, TrackHaloIds[i], SubCM[0][i], SubCM[1][i],
                    SubCM[2][i], SubCMV[0][i], SubCMV[1][i], SubCMV[2][i]);

        /* 				else */
        /* 				  fprintf(stderr,"Nfound_in_group[%"STR_FMT"]
         * = %"STR_FMT"\n",i,Nfound_in_Group[i]); */
