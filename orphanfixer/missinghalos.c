...
void print_header_ghost_halos(FILE *fp)
{
    ...
    fprintf(fp, "####################################################################"
                "################################################################\n");
    fprintf(fp, "#     snapshot         haloid          xcen            ycen         "
...
void output_missing_halo_centres(struct node_data *tree[], int64 *Ngroups)
{
    ...
    my_snprintf(snapshot_name, MAXLEN, "%s/%s_%03d", PARAMS.SNAPSHOT_DIR, PARAMS.SNAPSHOT_BASE,
    ...
    if (check_totnpart != TOTNPART)
    {
        ...
        fprintf(stderr,
                "Got TOTNPART  = %" PRId64 " from COSMO struct but snapshot header shows %" PRId64 " particles\n",
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (tmpnode != NULL && (thisnode->snapshot - tmpnode->snapshot) > 1)
                {
                    ...
                    Track_NumPart_in_Group =
                        my_realloc(Track_NumPart_in_Group, sizeof(int64), Nids, "Track_NumPart_in_Group");
                    EarliestSnapshot_to_Track =
                        my_realloc(EarliestSnapshot_to_Track, sizeof(short), Nids, "EarliestSnapshot_to_Track");

                    for (short missing_snapshot = isnapshot - 1; missing_snapshot >= tmpnode->snapshot + 1;
                         missing_snapshot--)
                    {
    ...
    {                                                                                                                  \
        ...
        SGLIB_ARRAY_ELEMENTS_EXCHANGER(int64, Track_NumPart_in_Group, i, j);                                           \
            ...
            while (snapshot >= EarliestSnapshot && snapshot > PARAMS.MIN_SNAPSHOT_NUM && Nids > 0)
            {
                ...
                SGLIB_ARRAY_QUICK_SORT(short, EarliestSnapshot_to_Track, Nids, SGLIB_NUMERIC_COMPARATOR,
                ...
                while (ifile < nfiles && totnleft > 0)
                {
                    ...
                    {
                        my_snprintf(snapshot_name, MAXLEN, "%s/%s_%03d.%d", PARAMS.SNAPSHOT_DIR, PARAMS.SNAPSHOT_BASE,
                                    snapshot, ifile);
                    }

                    fprintf(stderr,
                            "snapshotfile = `%s' Nmissing = %" STR_FMT " Nids = %" STR_FMT "  totnleft = %" STR_FMT
                            " Earliestsnap = %hd \n",
                    ...
                    for (int k = 0; k < 6; k++)
                    {
                        for (int n = 0; n < header1.npart[k]; n++)
                        {
                            ...
                            {
                                fprintf(stderr,
                                        "ERROR: I have id = %" STR_ID_FMT " but it should be between 0 and %" PRId64
                                        " \n",
                                        tmp_id, TOTNPART);
                                exit(EXIT_FAILURE);
                            }
                            ...
                            if (location != -1)
                            {
                                if (location >= Nids || location < 0)
                                {
                                    fprintf(stderr,
                                            ...
                                            "%" STR_FMT " tmp_id = %" STR_ID_FMT " index = %" STR_FMT "\n",
                                ...
                                if (TrackIds[location] != tmp_id)
                                {
                                    fprintf(stderr,
                                            ...
                                            "TrackIds[%" STR_FMT "] = %" STR_ID_FMT "  tmp_id = %" STR_ID_FMT "\n",
                                ...
                                else
                                {
                                    ...
                                    for (int ii = 0; ii < 3; ii++)
                                    {
                                        SubCM[ii][TrackNums[location]] +=
                                            periodic(tmp_pos[ii] - zero_part_pos[ii][TrackNums[location]]);
                    ...
                    fprintf(stderr,
                            "after reading the snapshot: totnleft = %" STR_FMT "  index = %" STR_FMT
                            " nmisses=%" STR_FMT " index-nmisses=%" STR_FMT "\n",
                ...
                for (int64 i = 0; i < Nmissing; i++)
                    if (Nfound_in_Group[i] > 0)
                        fprintf(fp, "%10hd     %14" STR_FMT "   %14.4f   %14.4f   %14.4f   %14.4f   %14.4f   %14.4f\n",
                                snapshot, TrackHaloIds[i], SubCM[0][i], SubCM[1][i], SubCM[2][i], SubCMV[0][i],
                                SubCMV[1][i], SubCMV[2][i]);
