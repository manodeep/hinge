...
void arepo_read_dataset(hid_t HDF_FileID, char *filename, char *gid, char *dataid, struct particle *p, int64_t to_read,
...
void load_particles_arepo(char *filename, struct particle **p, int64_t *num_p)
{
    ...
    TOTAL_PARTICLES = (((int64_t)npart_high[AREPO_DM_PARTTYPE]) << 32) + (int64_t)npart_low[AREPO_DM_PARTTYPE];
    ...
    arepo_read_dataset(HDF_FileID, filename, buffer, "ParticleIDs", *p + (*num_p), to_read,
                       (char *)&(p[0][0].id) - (char *)(p[0]), 1, H5T_NATIVE_LLONG);
    arepo_read_dataset(HDF_FileID, filename, buffer, "Coordinates", *p + (*num_p), to_read,
                       (char *)&(p[0][0].pos[0]) - (char *)(p[0]), 3, H5T_NATIVE_FLOAT);
    arepo_read_dataset(HDF_FileID, filename, buffer, "Velocities", *p + (*num_p), to_read,
