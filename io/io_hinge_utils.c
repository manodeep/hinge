...
//"# ID                  fofid                  nsub                  Mvir                  npart                  Xc                  Yc                  Zc                  VXc                  VYc                  VZc                  Rvir                  partid_start_bytes_offset                  partid_end_bytes_offset"

    char wanted_columns[][MAXLEN] = {"ID", "fofid", "nsub", "Mvir", "npart", "Xc", "Yc", "Zc", "VXc", "VYc", "VZc", "Rvir"};
    ...
    XASSERT(num_columns == num_column_indices, "Number of column names = %d does not match number of indices = %d", num_columns, num_column_indices);
    ...
    while ((token = strsep((char **) &buffer, " ")) != NULL) {
        ...
        XASSERT(strcasecmp(token, wanted_columns[colnum]) == 0, "Column name '%s' does not match wanted column name '%s'", token, wanted_columns[colnum]);
        colnum++;
    }
    ...
    while(fscanf(fp, "%"SCNd64" %"SCNd64" %"SCNd64" %lf %"SCNd64" %lf %lf %lf %lf %lf %lf %lf",
                &(halos->halo_id), &halos->fof_id, &halos->nsub, &halos->Mvir, &halos->npart, &halos->Xc, &halos->Yc, &halos->Zc, &halos->VXc, &halos->VYc, &halos->VZc, &halos->Rvir) == num_columns)
    {
