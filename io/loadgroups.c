...
#define MULTIPLE_ARRAY_EXCHANGER(vartype, a, i, j)                             \
  {                                                                            \
    ...
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(int64, thisgroup->parentgroupforparticle,   \
                                   i, j);                                      \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(int, thisgroup->parentsnapshotforparticle,  \
                                   i, j);                                      \
  }
...
#define MULTIPLE_ARRAY_EXCHANGER(vartype, a, i, j)                             \
  {                                                                            \
    ...
    SGLIB_ARRAY_HEAP_SORT(float, sqr_radius, thisgroup->N,
                          SGLIB_NUMERIC_COMPARATOR, MULTIPLE_ARRAY_EXCHANGER);
...
static inline void mark_particle_for_deletion(struct group_data *this_group,
                                              const int64 partloc) {
  assert(this_group != NULL &&
         "The group pointer can not be NULL while removing a particle");
  id64 *id = this_group->id;

  id[partloc] = -1;
}

static inline void swap_groups(struct group_data *groups, const int64 first,
                               const int64 second) {
...
void remove_particles_from_all_groups(struct group_data *groups,
                                      const int Ngroups) {
  ...
  for (int64 igroup = 0; igroup < Ngroups; igroup++) {
    ...
    if (num_moved > 0) {
      assert(this_group->N + num_moved == orig_N &&
             "Sum of particles removed and particles left should equal "
  ...
  for (int64 igroup = 0; igroup < Ngroups; igroup++) {
    ...
    for (int64 j = 0; j < this_group->N; j++) {
      if (!(*id != -1)) {
        ...
        fprintf(stderr,
                "igroup = %" STR_FMT " j = %" STR_FMT
                " this_group->N = %" STR_FMT "\n",
 ...
 */
void remove_duplicate_particles(const int64 Ngroups,
                                struct group_data *groups) {
  int64 FofIndex =
      0; // actual index and not a fof haloid since that can be different
  ...
  while (FofIndex < Ngroups) {
    ...
    for (int64 igroup = FofIndex; igroup < (FofIndex + groups[FofIndex].Nsub);
         igroup++) {
      if (!(groups[igroup].FOFHalo == FofIndex)) {
        fprintf(stderr,
                "ERROR: ABout to crash. igroup = %" STR_FMT
                " FofIndex = %" STR_FMT " group->FOFHalo = %" STR_FMT "\n",
                igroup, FofIndex, groups[igroup].FOFHalo);
      }
      assert(groups[igroup].FOFHalo == FofIndex &&
             "The FOFHalo indices should have been setup correctly");
      ...
      for (int64 j = 0; j < groups[igroup].N; j++) {
        ...
        if (location == max_npart) {
          ...
          halolevel =
              my_realloc(halolevel, sizeof(*halolevel), max_npart, "halolevel");
          haloindex =
              my_realloc(haloindex, sizeof(*haloindex), max_npart, "haloindex");
          partloc_in_halo =
              my_realloc(partloc_in_halo, sizeof(*partloc_in_halo), max_npart,
                         "partloc_in_halo");
        }
        ...
        assert(groups[igroup].ParentLevel >= 1 &&
               "ParentLevels need to have been initialized already");
...
#define MULTIPLE_ARRAY_EXCHANGER(vartype, a, i, j)                             \
  {                                                                            \
    ...
    SGLIB_ARRAY_QUICK_SORT(id64, partids, Npart, SGLIB_NUMERIC_COMPARATOR,
...
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    while (ipart < Npart) {
      ...
      for (int64 i = ipart; i <= (ipart + nmatches); i++) {
        ...
        if (halolevel[i] < maxparentlevel) {
          ...
          assert(this_haloindex >= 0 &&
                 this_haloindex < (FofIndex + groups[FofIndex].Nsub));
    ...
    assert(groups[FofIndex].Nsub >= 1 &&
           "Number of subhalos must be at least 1");
    FofIndex += groups[FofIndex].Nsub;
  }
  assert(FofIndex == Ngroups &&
         "The number of halos processed should be *exactly* equal to the "
...
void assign_parentlevel(struct group_data *this_group, const int64 Ngroups,
                        struct group_data *groups) {
  const int64 hosthaloindex = this_group->ContainerIndex;
  assert(hosthaloindex >= 0 && hosthaloindex < Ngroups &&
         "Host halo index must be within bounds");
  ...
  if (containerhalo->ParentLevel == -1) {
    if (!(containerhalo->ContainerIndex != hosthaloindex)) {
      fprintf(stderr,
              "ERROR: About to crash -- hosthalo  = %" STR_FMT
              " host->parentlevel = %d this->Nsub = %" STR_FMT
              " parentlevel = %d\n",
              hosthaloindex, containerhalo->ParentLevel, this_group->Nsub,
              this_group->ParentLevel);
    }
    assert(containerhalo->ContainerIndex != hosthaloindex &&
           "Bug in fixing container indices. If ParentLevel is not set, then "
           "halo can not contain itself - infinite recursion will occur");
    assign_parentlevel(containerhalo, Ngroups, groups);
  }

  assert(containerhalo->ParentLevel >= 1 &&
         "ParentLevels must be at least 1 (for FOFs) and larger for "
         "sub/sub-subs etc");
  ...
  if (fof_only != 1) {
    ...
    for (int64 i = 0; i < Ngroups; i++) {
      if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
        ...
        fprintf(stderr,
                "ERROR: Could not parse the halo file `%s' for %ld values\n",
                buf, Ngroups);
        exit(EXIT_FAILURE);
      }
    }
...
void loadgroups(int num, struct group_data *group) {
  ...
  long haloid;
  char buffer[MAXLINESIZE];
  my_snprintf(catalogue_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_halos",
              PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num, REDSHIFT[num]);
  my_snprintf(partids_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_particles",
              PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num, REDSHIFT[num]);
  my_snprintf(substruct_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_substructure",
              PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num, REDSHIFT[num]);
  ...
  while (i < Ngroups) {
    if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
      if (buffer[0] != '#') {
        ...
        const int nread = sscanf(
            buffer,
            "%ld %" STR_FMT " %" STR_FMT " %lf %" STR_FMT " %f %f %f %f %f %f",
            &group[i].haloID, &group[i].FOFHalo, &group[i].Nsub, &group[i].Mtot,
            &group[i].N, &group[i].xcen, &group[i].ycen, &group[i].zcen,
            &group[i].vxcen, &group[i].vycen, &group[i].vzcen);
  ...
  /* We have read in the entire halo catalog - however, the subhalos and the
     hosts are not contiguous yet. First make sure that the halos appear FOF,
     sub, sub.., next FOF, sub...etc

           Easiest way to do this is to sort on FofHalo field of the group
     struct using sglib
...
#error Not implemented.
  ...
  while (i < Ngroups) {
    if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
      if (buffer[0] != '#') {
        /* fprintf(stderr,"group[%"STR_FMT"].haloid = %ld buffer = `%s'
         * ",i,group[i].haloID,buffer); */
        ...
        // group[i].type           = my_malloc(sizeof(*group[i].type)
        // ,group[i].N);
        group[i].ParticleEnergy =
            my_malloc(sizeof(*group[i].ParticleEnergy), group[i].N);
        ...
        group[i].parentgroupforparticle =
            my_malloc(sizeof(*group[i].parentgroupforparticle), group[i].N);
        group[i].parentsnapshotforparticle =
            my_malloc(sizeof(*group[i].parentsnapshotforparticle), group[i].N);

        for (int64 j = 0; j < group[i].N; j++) {
          if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
            // group[i].type[j] = 1;
            sscanf(buffer, "%ld %f %f %f %f %f %f %f", &(group[i].id[j]),
            ...
            fprintf(stderr,
                    "ERROR: Could not parse file for individual particles \n");
            exit(EXIT_FAILURE);
          }
        }
        i++;
...
void loadgroups(int num, struct group_data *group) {
  ...
  /* #ifndef ASCII_DATA */
  /*   my_snprintf(catalogue_fname,MAXLEN,  "%s/%s_%03d/subhalo_tab_%03d.0",
   * PARAMS.GROUP_DIR, PARAMS.GROUP_BASE,num,num); */
  /*   my_snprintf(partids_fname,MAXLEN,  "%s/%s_%03d/subhalo_ids_%03d.0",
   * PARAMS.GROUP_DIR, PARAMS.GROUP_BASE,num,num); */
  /* #else */
  long Ngroups;
  char buffer[MAXLINESIZE];
  my_snprintf(catalogue_fname, MAXLEN, "%s/%s%05d.z%5.3f.AHF_halos",
              PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num, REDSHIFT[num]);
  my_snprintf(partids_fname, MAXLEN, "%s/%s%05d.z%5.3f.AHF_particles",
              PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num, REDSHIFT[num]);
  /* my_snprintf(substruct_fname,MAXLEN,"%s/%s%05d.z%5.3f.AHF_substructure"   ,
   * PARAMS.GROUP_DIR, PARAMS.GROUP_BASE,num,REDSHIFT[num]); */
    ...
    if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
      if (buffer[0] != '#') {
        ...
        /* Increment the number of subhalos since we need to count the halo
                 itself as one subhalo. This way, even if the halo has no other
           subhalos, the halo itself is counted. There are numerous loops
           scattered around in the code that depend on Nsub>=1.
         */
        ...
        if (group[i].FOFHalo == 0) {
          ...
          group[i].isFof = 0;
          // This was the SUSSING mergertree way of storing the FOF host
          // information
          ...
          /* This might not be true -- since the host halo might actually not be
             the FOF we are after and it might be true that the actual FOF halo
             has not been read-in yet ( I don't think that's the case but I will
             assume that to be safe).
           */
          group[i].FOFHalo = -1;
        }
        i++;
  ...
  fgets(buffer, MAXLINESIZE, fp);
  sscanf(buffer, "%ld", &Ngroups);
  fprintf(stderr, "Reading particles from file  = `%s' Ngroups = %ld ...",
  ...
  while (i < Ngroups) {
    if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
      if (buffer[0] != '#') {
        ...
        // Since I have re-ordered the groups before, this check will not be
        // valid any longer
        /* assert(haloid == group[i].haloID); */
        ...
        /* haloid now contains the index of the group in question. Check that
         * that is true */
        assert(haloindex >= 0 && haloindex < Ngroups &&
               "Checking that halo index is within bounds");
        if (!(haloindex >= 0 && haloindex < Ngroups &&
              haloid == group[haloindex].haloID &&
              numpart_in_halo == group[haloindex].N)) {
          fprintf(stderr, "ERROR: About to crash. Here's some info about the "
                          "group/particle that caused the crash\n");
          fprintf(stderr,
                  " i = %" STR_FMT
                  " haloid = %ld haloindex = %ld Ngroups = %" STR_FMT
                  " numpart_in_halo = %" STR_FMT "\n",
                  i, haloid, haloindex, Ngroups, numpart_in_halo);
          fprintf(stderr,
                  "group[haloindex].haloid = %" STR_FMT " group.N = %" STR_FMT
                  "\n",
                  group[haloindex].haloID, group[haloindex].N);
        }

        assert(haloindex >= 0 && haloindex < Ngroups &&
               "haloindex must be valid");
        assert(haloindex >= 0 && haloindex < Ngroups &&
               "haloindex must be valid");
        assert(
            haloid == group[haloindex].haloID &&
            "Checking that haloid matches between particles and halo catalogs");
        assert(numpart_in_halo == group[haloindex].N &&
               "Checking that the number of particles match between particle "
               "and halo catalogs");

        /* Now allocate memory and prepare to parse the particles file for the
         * particle positions in the group */
        group[haloindex].id =
            my_malloc(sizeof(*group[haloindex].id), group[haloindex].N);
        // group[haloindex].type           =
        // my_malloc(sizeof(*group[haloindex].type) ,group[haloindex].N);
        group[haloindex].ParticleEnergy = my_malloc(
            sizeof(*group[haloindex].ParticleEnergy), group[haloindex].N);
        group[haloindex].x =
            my_malloc(sizeof(*group[haloindex].x), group[haloindex].N);
        group[haloindex].y =
            my_malloc(sizeof(*group[haloindex].y), group[haloindex].N);
        group[haloindex].z =
            my_malloc(sizeof(*group[haloindex].z), group[haloindex].N);
        group[haloindex].vx =
            my_malloc(sizeof(*group[haloindex].vx), group[haloindex].N);
        group[haloindex].vy =
            my_malloc(sizeof(*group[haloindex].vy), group[haloindex].N);
        group[haloindex].vz =
            my_malloc(sizeof(*group[haloindex].vz), group[haloindex].N);

        group[haloindex].parentgroupforparticle =
            my_malloc(sizeof(*group[haloindex].parentgroupforparticle),
                      group[haloindex].N);
        group[haloindex].parentsnapshotforparticle =
            my_malloc(sizeof(*group[haloindex].parentsnapshotforparticle),
                      group[haloindex].N);

        for (int64 j = 0; j < group[haloindex].N; j++) {
          if (fgets(buffer, MAXLINESIZE, fp) != NULL) {
            // group[haloindex].type[j] = 1;//This is incorrect because
            // particles might of various types
            nread = sscanf(
                buffer, "%ld %f %f %f %f %f %f %f", &(group[haloindex].id[j]),
  ...
  for (int64 igroup = 0; igroup < Ngroups; igroup++) {
    if (group[igroup].ParentLevel == 1)
      continue; // FOF halos already have parentlevels assigned -- no need to
                // process them
  ...
  /* for(int64 igroup=0;igroup<Ngroups;igroup++) { */
  /* 	fprintf(stderr,"igroup %07"STR_FMT" group->haloID = %10"STR_FMT"
   * group->N = %10"STR_FMT" group->FOFHalo = %10"STR_FMT" group->ContainerIndex
   * = %10"STR_FMT" group->parentlevel = %d group->Nsub=%"STR_FMT"\n" , */
  /* 					igroup,group[igroup].haloID,
   * group[igroup].N,group[igroup].FOFHalo,group[igroup].ContainerIndex,group[igroup].ParentLevel,group[igroup].Nsub);
   */
  /* } */
  ...
  for (int64 igroup = 0; igroup < Ngroups; igroup++) {
    ...
    /* fprintf(stderr,"igroup = %"STR_FMT" fofindex = %"STR_FMT"
     * group[igroup].ContainerIndex =
     * %"STR_FMT"\n",igroup,fofindex,group[igroup].ContainerIndex); */
    while (group[fofindex].isFof != 1) {
      /* fprintf(stderr,"[LOOP] igroup = %"STR_FMT" fofindex = %"STR_FMT"
       * group[igroup].ContainerIndex = %"STR_FMT" group[igroup].FOFHalo =
    ...
    assert(fofindex >= 0 && fofindex < Ngroups &&
           "Fofindex must be valid within [0,Ngroups)");
  ...
  /* Now rearrange the groups so that the Fofs and their associated subhalos are
   * contiguous */
#define MULTIPLE_ARRAY_EXCHANGER(vartype, a, i, j)                             \
  {                                                                            \
  ...
  SGLIB_ARRAY_QUICK_SORT(long, allfofhaloids, Ngroups, SGLIB_NUMERIC_COMPARATOR,
                         MULTIPLE_ARRAY_EXCHANGER);
#undef MULTIPLE_ARRAY_EXCHANGER

  /* One more re-ordering to make sure that the FOFs come first and then the
   * subhalos rather than some scrambled order (but all hosthaloindices must be
   * contiguous because of the sorting step */
  {
    int64 start = 0;
    while (start < Ngroups) {
      int64 end = start + 1;
      int64 fofloc =
          start; // assume that the fof is the first -- will get updated in the
                 // next while loop if that is not true
      ...
      while (end < Ngroups && allfofhaloids[end] == start_fof) {
        if (group[end].isFof == 1)
          fofloc = end;
        end++;
      }
      assert(fofloc >= 0 && fofloc < Ngroups &&
             "FOF groups must be valid index");
      assert(group[fofloc].isFof == 1 && "Fof location must actually be a fof");
      if (fofloc != start) {
        /* fprintf(stderr,"Before swap: NOT FOF  group[%"STR_FMT"].isFof == %d
         * Nsub = %"STR_FMT". FOF group[%"STR_FMT"].isFof = %d Nsub = %"STR_FMT"
         * end-start =
        ...
        /* fprintf(stderr,"After swap: FOF  group[%"STR_FMT"].isFof == %d
         * Nsub=%"STR_FMT". NOT FOF group[%"STR_FMT"].isFof = %d Nsub =
         * %"STR_FMT" end-start =
         * %"STR_FMT"\n",start,group[start].isFof,group[start].Nsub,fofloc,group[fofloc].isFof,group[fofloc].Nsub,end-start);
         */
      }
      assert(group[start].isFof == 1 && "FOF must be at the correct index");
      group[start].Nsub = end - start; // Update the number of subhalos to
                                       // contain all the sub-subs etc
      /* fprintf(stderr,"start = %"PRId64" end = %"PRId64"  Nsub = %"PRId64"
       * start+Nsub =
       * %"PRId64"\n",start,end,group[start].Nsub,start+group[start].Nsub); */
      /* for(i=start;i<end;i++) { */
      /* 	fprintf(stderr,"i=%"PRId64" isfof=%d Nsub=%"PRId64"
  ...
  for (int64 fofindex = 0; fofindex < Ngroups;
       fofindex += group[fofindex].Nsub) {
    if (!(group[fofindex].isFof == 1)) {
      fprintf(stderr,
              "ERROR: About to crash - Fof halo doesn't have fof flag set. "
              "fofindex = %" STR_FMT " Nsub = %" STR_FMT " haloid = %" STR_FMT
              " Parentlevel = %d ContainerIndex = %" STR_FMT "\n",
              fofindex, group[fofindex].Nsub, group[fofindex].haloID,
              group[fofindex].ParentLevel, group[fofindex].ContainerIndex);
    }
    assert(group[fofindex].isFof == 1 && "fofindex must point to a FOF group");
    assert(group[fofindex].Nsub >= 1 &&
           "Number of subhalos must be at least 1");
    group[fofindex].ContainerIndex = fofindex;
    for (int64 igroup = fofindex; igroup < (fofindex + group[fofindex].Nsub);
         igroup++) {
       ...
       */
      int64 jgroup;
      for (jgroup = fofindex; jgroup < (fofindex + group[fofindex].Nsub);
           jgroup++) {
        /* fprintf(stderr,"fofindex = %"PRId64" Nsub = %"PRId64" igroup =
         * %"PRId64" jgroup = %"PRId64" orig_containerindex = %"PRId64"
         * currentindices[jgroup] = %"PRId64" group[jgroup].ParentLev = %d
         * group[igroup].ParentLev = %d\n", */
      ...
      if (igroup == fofindex) {
        if (!(jgroup == fofindex)) {
          fprintf(stderr,
                  "ERROR: About to crash. igroup = %" STR_FMT
                  " jgroup = %" STR_FMT " fofindex = %" STR_FMT
                  " groupnum = %" STR_FMT " isfof = %d\n",
                  igroup, jgroup, fofindex, group[igroup].groupnum,
                  group[igroup].isFof);
        }
        assert(jgroup == fofindex && "Bug in fixing container indices. FOF "
                                     "must refer to itself as the container");
      }
      assert(currentindices[jgroup] == orig_containerindex &&
             "ERROR: The impossible has happened. Could not find container "
             "index for halo");
  ...
  print_time(t_sectionstart, t_sectionend,
             "Re-ordering particles inside groups");
}
#endif // SUSSING
...
void loadgroups(int num, struct group_data *group) {
  ...
  char partids_fname[MAXLEN];
  my_snprintf(particles_fname, MAXLEN, "%s/%s_%03d.pos", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  // my_snprintf(parttypes_fname, MAXLEN, "%s/%s_%03d.types", PARAMS.GROUP_DIR,
  // PARAMS.GROUP_BASE, num);
  my_snprintf(partids_fname, MAXLEN, "%s/%s_%03d.ids", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  my_snprintf(catalogue_fname, MAXLEN, "%s/%s_%03d.fofcat", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  ...
  assert(xx == NumPart &&
         "Numpart in ids file should be equal to NumPart in positions file");
  ...
  my_snprintf(subhalos_fname, MAXLEN, "%s/%s_%03d.subcat", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  my_snprintf(subprop_fname, MAXLEN, "%s/%s_%03d.subprop", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  ...
  my_snprintf(fofprop_fname, MAXLEN, "%s/%s_%03d.fofprop", PARAMS.GROUP_DIR,
              PARAMS.GROUP_BASE, num);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    my_fread(GroupCMV[i], sizeof(float), Ngroups,
             fprop); /* Warning: This was written directly as a block of 3. I am
                        reading it back in as 3 sets of 1. */
#endif
  }
  ...
  */
  ...
  for (int64 i = 0; i < Ngroups; i++) {
    if (tempindex[i] >= Nsub) {
      fprintf(stderr, "WARNING: Possible case of FOF group falling below "
                      "resolution limit \n");
      fprintf(stderr,
              "WARNING: i = %" STR_FMT " tempindex = %" STR_FMT
              " is greater than Nsub = %" STR_FMT " with Ngroups = %" STR_FMT
              "  \n",
  ...
  for (int64 i = 0; i < Nsub; i++) {
    ...
    */
    ...
    group[i].parentgroupforparticle =
        my_malloc(sizeof(*group[i].parentgroupforparticle), SubLen[i]);
    group[i].parentsnapshotforparticle =
        my_malloc(sizeof(*group[i].parentsnapshotforparticle), SubLen[i]);
    ...
    for (int64 j = 0; j < SubLen[i]; j++) {
      ...
      float temp_rmax = sqrt(SQR_PERIODIC(group[i].x[j] - group[i].xcen) +
                             SQR_PERIODIC(group[i].y[j] - group[i].ycen) +
  ...
  for (int64 i = 0; i < Ngroups; i++) {
    ...
    group[i].parentgroupforparticle =
        my_malloc(sizeof(*group[i].parentgroupforparticle), GroupLen[i]);
    group[i].parentsnapshotforparticle =
        my_malloc(sizeof(*group[i].parentsnapshotforparticle), GroupLen[i]);
