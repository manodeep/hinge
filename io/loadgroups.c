...
void assign_parentlevel(struct group_data *this_group, const int64 Ngroups, struct group_data *groups)
{
    ...
    if (containerhalo->ParentLevel == -1)
    {
        ...
        {
            fprintf(stderr,
                    "ERROR: About to crash -- hosthalo  = %" STR_FMT " host->parentlevel = %d this->Nsub = %" STR_FMT
                    " parentlevel = %d\n",
    ...
    assert(containerhalo->ParentLevel >= 1 && "ParentLevels must be at least 1 (for FOFs) and larger for "
                                              "sub/sub-subs etc");
...
void loadgroups(int num, struct group_data *group)
{
    ...
    long haloid;
    char buffer[MAXLINESIZE];
    my_snprintf(catalogue_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_halos", PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num,
                REDSHIFT[num]);
    my_snprintf(partids_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_particles", PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num,
                REDSHIFT[num]);
    my_snprintf(substruct_fname, MAXLEN, "%s/%s_%03d.z%5.3f.AHF_substructure", PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num,
                REDSHIFT[num]);
    ...
    while (i < Ngroups)
    {
        if (fgets(buffer, MAXLINESIZE, fp) != NULL)
        {
            if (buffer[0] != '#')
            {
                ...
                const int nread =
                    sscanf(buffer, "%ld %" STR_FMT " %" STR_FMT " %lf %" STR_FMT " %f %f %f %f %f %f", &group[i].haloID,
                           &group[i].FOFHalo, &group[i].Nsub, &group[i].Mtot, &group[i].N, &group[i].xcen,
                           &group[i].ycen, &group[i].zcen, &group[i].vxcen, &group[i].vycen, &group[i].vzcen);
...
void loadgroups(int num, struct group_data *group)
{
    ...
    long Ngroups;
    char buffer[MAXLINESIZE];
    my_snprintf(catalogue_fname, MAXLEN, "%s/%s%05d.z%5.3f.AHF_halos", PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num,
                REDSHIFT[num]);
    my_snprintf(partids_fname, MAXLEN, "%s/%s%05d.z%5.3f.AHF_particles", PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, num,
                REDSHIFT[num]);
    ...
    while (i < Ngroups)
    {
        if (fgets(buffer, MAXLINESIZE, fp) != NULL)
        {
            if (buffer[0] != '#')
            {
                ...
                if (!(haloindex >= 0 && haloindex < Ngroups && haloid == group[haloindex].haloID &&
                      numpart_in_halo == group[haloindex].N))
                {
                    ...
                    fprintf(stderr,
                            " i = %" STR_FMT " haloid = %ld haloindex = %ld Ngroups = %" STR_FMT
                ...
                assert(haloindex >= 0 && haloindex < Ngroups && "haloindex must be valid");
                assert(haloid == group[haloindex].haloID &&
                ...
                group[haloindex].ParticleEnergy =
                    my_malloc(sizeof(*group[haloindex].ParticleEnergy), group[haloindex].N);
                ...
                group[haloindex].parentgroupforparticle =
                    my_malloc(sizeof(*group[haloindex].parentgroupforparticle), group[haloindex].N);
                group[haloindex].parentsnapshotforparticle =
                    my_malloc(sizeof(*group[haloindex].parentsnapshotforparticle), group[haloindex].N);
                ...
                for (int64 j = 0; j < group[haloindex].N; j++)
                {
                    if (fgets(buffer, MAXLINESIZE, fp) != NULL)
                    {
                        ...
                        nread = sscanf(buffer, "%ld %f %f %f %f %f %f %f", &(group[haloindex].id[j]),
...
#undef MULTIPLE_ARRAY_EXCHANGER
     ...
     * contiguous because of the sorting step */
    {
        ...
        {
            int64 end = start + 1;
            int64 fofloc = start; // assume that the fof is the first -- will get updated in the
    ...
    for (int64 fofindex = 0; fofindex < Ngroups; fofindex += group[fofindex].Nsub)
    {
        if (!(group[fofindex].isFof == 1))
        {
            fprintf(stderr,
                    ...
                    fofindex, group[fofindex].Nsub, group[fofindex].haloID, group[fofindex].ParentLevel,
                    group[fofindex].ContainerIndex);
        }
        ...
        for (int64 igroup = fofindex; igroup < (fofindex + group[fofindex].Nsub); igroup++)
        {
            ...
            if (igroup == fofindex)
            {
                ...
                {
                    fprintf(stderr,
                            "ERROR: About to crash. igroup = %" STR_FMT " jgroup = %" STR_FMT " fofindex = %" STR_FMT
