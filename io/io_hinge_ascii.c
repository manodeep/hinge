...
int64 returnNhalo_hinge_ascii(const struct params_data *params, const int snapnum, const int fof_only)
{
    ...
    my_snprintf(catalogue_fname, MAXLEN, "%s/hinge_halos_z%0.3f.txt", params->GROUP_DIR, params->GROUP_BASE,
                REDSHIFT[snapnum]);
...
#define MAXBUFSIZE 10000
    ...
    my_snprintf(particles_fname, MAXLEN, "%s/hinge_particles_z%0.3f.txt", params->GROUP_DIR, params->GROUP_BASE,
                REDSHIFT[snapnum]);
    my_snprintf(catalogue_fname, MAXLEN, "%s/hinge_halos_z%0.3f.txt", params->GROUP_DIR, params->GROUP_BASE,
                REDSHIFT[snapnum]);
