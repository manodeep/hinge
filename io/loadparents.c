  ...
  while (1) {
    if (fgets(str_line, MAXLINESIZE, fp) != NULL) {
      if (str_line[0] != comment) {
        ...
        sscanf(str_line,
               "%hd   %" STR_FMT " %hd  %" STR_FMT " %" STR_FMT "  %" STR_FMT
               "  %" STR_FMT " %hd        %" STR_FMT " %lf   %" STR_FMT " \n",
               &parent[nlines].snapshot, &parent[nlines].groupid,
               &parent[nlines].parentlevel, &parent[nlines].containerid,
               &parent[nlines].nsub, &parent[nlines].npartinhalo,
               &parent[nlines].parentid, &parent[nlines].parentsnapshot,
               &parent[nlines].npartinparent, &parent[nlines].rank,
               &parent[nlines].ncommon);
