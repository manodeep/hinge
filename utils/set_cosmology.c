...
void set_cosmology(struct cosmology_data *CP)
{
  ...
  CP->Omega_rad = 0.0;
  CP->Omega_k   = 0.0;
  CP->K_fit = 3.6; /* fit for overdensity of 200 and all halos. not just relaxed ones. */
  CP->F_fit = 0.01;
#endif
...
{
  double ez = sqrt(PARAMS.COSMO->Omega_m*(1.0+z)*(1.0+z)*(1.0+z) +  PARAMS.COSMO->Omega_lambda);/*assumes flat Universe with only matter and lambda*/
  return ez;
}
...
float get_overdensity(float z,struct cosmology_data *CP)
{
  ...
  */
  ...
  float omega,Ez,x;
  Ez = sqrt( CP->Omega_lambda + CP->Omega_k* SQR(1.0+z) + CP->Omega_m*CUBE(1.0+z)  + CP->Omega_rad*SQR(SQR(1.0+z)));
  omega = CP->Omega_m*CUBE(1.0+z)/SQR(Ez);
  x = omega - 1.0;
  return (18.0*SQR(PI) + 18.0*x - 39.0*SQR(x))/omega; /* As explained in Bullock 2001 paper (for that factor of omega in the denom.) */
...
double get_hubble_at_redshift(float z,struct cosmology_data *CP)
{
  ...
  hubble = sqrt( CP->Omega_lambda + CP->Omega_k*oneplusz_sqr + CP->Omega_m*oneplusz*oneplusz_sqr  + CP->Omega_rad*oneplusz_sqr*oneplusz_sqr) *CP->H0*1e3;
  return hubble;
}
  ...
  if(i < nbins && i > 1 && rho[i] > halfmass && rho[i-1] < halfmass)
	{
	  ...
	  group->Rhalf = r1*(halfmass - rho[i-1]) + r_minus1*(rho[i] - halfmass);/* linear interpolation to half-mass radius */
