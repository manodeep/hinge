...
double get_age(float z) {
  ...
  const double AGE_AT_RECOMBINATION =
      0.37 * 1e-3; /*in Gyr ( recombination = 0.37 Myr)*/
  ...
  F.function = &agefunc;
  F.params = &dummy;
  gsl_integration_qags(&F, z, RECOMBINATION_REDSHIFT, 0, 1e-7, NWORKSPACE, w,
                       &result, &error);
...
double epeebles(float z) {
  double ez = sqrt(
      PARAMS.COSMO->Omega_m * (1.0 + z) * (1.0 + z) * (1.0 + z) +
      PARAMS.COSMO
          ->Omega_lambda); /*assumes flat Universe with only matter and lambda*/
  return ez;
}
...
float get_overdensity(float z, struct cosmology_data *CP) {
  ...
  */
  ...
  float omega, Ez, x;
  Ez = sqrt(CP->Omega_lambda + CP->Omega_k * SQR(1.0 + z) +
            CP->Omega_m * CUBE(1.0 + z) + CP->Omega_rad * SQR(SQR(1.0 + z)));
  omega = CP->Omega_m * CUBE(1.0 + z) / SQR(Ez);
  x = omega - 1.0;
  return (18.0 * SQR(PI) + 18.0 * x - 39.0 * SQR(x)) /
         omega; /* As explained in Bullock 2001 paper (for that factor of omega
                   in the denom.) */
}
...
double get_hubble_at_redshift(float z, struct cosmology_data *CP) {
  ...
  hubble = sqrt(CP->Omega_lambda + CP->Omega_k * oneplusz_sqr +
                CP->Omega_m * oneplusz * oneplusz_sqr +
                CP->Omega_rad * oneplusz_sqr * oneplusz_sqr) *
           CP->H0 * 1e3;
  return hubble;
}

double getrvir_anyl(const double mvir, const float z,
                    struct cosmology_data *CP) {
  double rvir;
...
#ifdef WMAP5
  conc = pow(
      10.0, 0.917 - 0.104 * log10(mvir / 1e2)); /*compute pretending to be z=0*/
#endif
...
void getrvir_from_overdensity(struct group_data *group, int NBINS,
                              const double RhoCrit, const float OverDensity) {
  ...
  */
  ...
  if (Npart <= 1) {
    fprintf(stderr,
            "Number of particles needs to be greated than 1 ..exiting \n\n");
    exit(EXIT_FAILURE);
  } else if (Npart <= 100)
    nbins = 20;
  ...
  if (group->Mtot > 0 &&
    ...
    fprintf(stderr,
            ...
            Npart, group->Mtot, PARAMS.MASSARR[DM_PART_TYPE] * Npart,
            PARAMS.MASSARR[DM_PART_TYPE]);
    exit(EXIT_FAILURE);
  }
  ...
  ycen = group->ycen;
  zcen = group->zcen;
  assert(xcen >= 0 && xcen <= PARAMS.BOXSIZE &&
         "xcen must be in [0.0, BoxSize]");
  assert(ycen >= 0 && ycen <= PARAMS.BOXSIZE &&
         "ycen must be in [0.0, BoxSize]");
  assert(zcen >= 0 && zcen <= PARAMS.BOXSIZE &&
         "zcen must be in [0.0, BoxSize]");
  ...
  */
  ...
  for (i = 1; i < Npart; i++) {
    ...
    numberdensity[index] +=
        1; /* Should be used as error estimation for fitting */
  ...
  if (i < nbins && i > 1 && rho[i] > halfmass && rho[i - 1] < halfmass) {
    ...
    group->Rhalf =
        r1 * (halfmass - rho[i - 1]) +
        r_minus1 *
            (rho[i] - halfmass); /* linear interpolation to half-mass radius */
  ...
  for (i = 0; i < nbins; i++) {
    ...
    rho[i] /=
        (4. / 3. * PI *
         pow(rbin, 3.0)); /* computes average density and NOT actual density */
    rho[i] /=
        RhoCrit; /* Convert rho to overdensity  -- have to set Cosmology first*/
  }
