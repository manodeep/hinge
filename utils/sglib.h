...
#define SGLIB_ARRAY_QUICK_SORT(type, a, max, comparator, elem_exchanger)                                               \
    {                                                                                                                  \
        ...
        int _startStack_[64];                                                                                          \
        int _endStack_[64];                                                                                            \
        /*   type  _tmp_;\ Commented out by Manodeep Sinha, 6th April. The                                             \
         * continuing slash is important */                                                                            \
        _startStack_[0] = 0;                                                                                           \
        _endStack_[0] = (max);                                                                                         \
        _stacki_ = 1;                                                                                                  \
        while (_stacki_ > 0)                                                                                           \
...
#define SGLIB___DL_LIST_GENERIC_ADD_IF_NOT_MEMBER(type, list, elem, comparator, previous, next, member,                \
                                                  the_add_operation)                                                   \
    {                                                                                                                  \
        type *_dlp_;                                                                                                   \
...
#define SGLIB_DEFINE_HASHED_TABLE_PROTOTYPES(type, dim, hash_function, comparator)                                     \
    ...
    extern type *sglib_hashed_##type##_it_init_on_equal(struct sglib_hashed_##type##_iterator *it, type *table[dim],   \
...
#define SGLIB_DEFINE_HASHED_TABLE_FUNCTIONS(type, dim, hash_function, comparator)                                      \
    ...
    type *sglib_hashed_##type##_it_init_on_equal(struct sglib_hashed_##type##_iterator *it, type *table[dim],          \
                                                 int (*subcomparator)(type *, type *), type *equalto)                  \
    {                                                                                                                  \
        int i;                                                                                                         \
        it->table = table;                                                                                             \
...
#define SGLIB_DEFINE_HASHED_CONTAINER_FUNCTIONS(type, dim, hash_function)                                              \
    ...
    {                                                                                                                  \
        ...
        it->subcomparator = subcomparator;                                                                             \
        it->equalto = equalto;                                                                                         \
        e = sglib_##type##_it_init_on_equal(&it->containerIt, table[it->currentIndex], it->subcomparator,              \
                                            it->equalto);                                                              \
        if (e == NULL)                                                                                                 \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            e = sglib_##type##_it_init_on_equal(&it->containerIt, it->table[it->currentIndex], it->subcomparator,      \
                                                it->equalto);                                                          \
        }                                                                                                              \
        return (e);                                                                                                    \
    }
