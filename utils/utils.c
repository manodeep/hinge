...
void my_fread(void *ptr, size_t size, size_t nmemb, FILE *stream) {
  ...
  if (nread != nmemb) {
    ...
    fprintf(stderr,
            "Instead of reading nmemb=%zu, I got nread = %zu ..exiting\n",
            nmemb, nread);
    exit(EXIT_FAILURE);
  }
}
...
void *my_malloc(size_t size, int64 N) {
  ...
  if (x == NULL) {
    fprintf(stderr,
            "malloc for %" STR_FMT
            "  elements with %zu size failed..aborting\n",
            N, size);
    exit(EXIT_FAILURE);
  } else if (megabytes > 100)
    fprintf(stderr,
            "\n Successfully allocated  %" STR_FMT
            " elements with total size %lu (MB) \n",
            N, megabytes);
...
void *my_calloc(size_t size, int64 N) {
  ...
  if (x == NULL) {
    fprintf(stderr,
            "malloc for %" STR_FMT " elements with %zu size failed..aborting\n",
            N, size);
    exit(EXIT_FAILURE);
  }
...
void *my_realloc(void *x, size_t size, int64_t N, const char *varname) {
  ...
  if (tmp == NULL) {
    ...
    if (gigabytes > 1)
      fprintf(stderr,
              "\n Successfully re-allocated  %" PRId64
              " elements with total size %zu (GB) for variable `%s' \n",
              N, gigabytes, varname);
  }
  return tmp;
}
...
struct group_data *allocate_group(int64 N) {
  ...
  if (g == NULL) {
    fprintf(stderr,
            "Error: Could not allocate memory for %" STR_FMT
            " elements for current snapshot \n",
            N);
    exit(EXIT_FAILURE);
  } else
...
int64 get_ncommon(struct group_data *prev, struct group_data *next) {
  ...
  PrevAllPartIds =
      my_calloc(sizeof(*PrevAllPartIds),
                PrevMaxPartId); /* Note use of calloc instead of malloc */
  for (int64 i = 0; i < prev->N; i++) {
    const id64 this_id = prev->id[i];
    assert(this_id < PrevMaxPartId &&
           "Particle ID must be smaller than max. particle ID");
