...
double assign_stellar_mass_from_mvir(struct node_data *const thisnode, int model)
{
    ...
    const char modelnames[][MAXLEN] = {"Conroy & Wechsler (2009)", "Moster et al (2010)", "Behroozi et al (2010)",
                                       "Behroozi et al (2013)"};
        ...
        if (z <= MinZ_for_Models[model])
        {
            ...
            case 3: // Behroozi (2013)
            {
                ...
                double log10_epsilon =
                    -1.777 + (-0.006 * (scale_factor - 1.0) + (-0.0) * z) * nu + (-0.119 * (scale_factor - 1.0));
        ...
        */
        ...
        while (thisnode != NULL && thisnode->haloid == node->haloid)
        {
            fprintf(fp, "%8d    %14" STR_FMT "    %14" STR_FMT "   %14.4g   %8d\n", thisnode->snapshot,
                    thisnode->nodeloc, thisnode->haloid, thisnode->Mtot, tag);
            thisnode = thisnode->Parent;
        }
    }
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (thisnode->haloid < 0)
                {
                    ...
                    while (thisnode != NULL)
                    {
                        if (thisnode->isFof == 1)
                        {
                            infallmass = thisnode->Mtot; // BoundFofMtot could also be used in
...
void find_mergers(struct node_data *tree[], int64 *Ngroups)
{
    ...
    double Impulse = 0.0;
    double DelPotPrim = 0.0, DelPotSec = 0.0, MaxDelPotPrim = 0.0, MaxDelPotSec = 0.0, tmpmaxdelpotsec = 0.0,
           tmpmaxdelpotprim = 0.0;
    ...
    fprintf(fp, "####################################################################"
                ...
                "####################################################################"
                "################################################\n");
    fprintf(fp, "#   Snapshot       ParentGrpnum      ParentHaloid    ParentMtot       "
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                                ...
                                if (savenode->haloid == thisnode->haloid)
                                {
                                    ...
                                    if (tag == 3 || tag == 5)
                                    {
                                        ...
                                        fprintf(fp2, "%e %e %e %e %e %e %e %e %e %" STR_FMT " %" STR_FMT "\n",
                                                savenode->z, savenode->FofHalo->BoundFofMtot, thisnode->InfallMass,
                                    ...
                                    while (tmpnode->Parent != NULL && tmpnode->Parent->haloid == thisnode->haloid &&
                                           tmpnode->Parent->FofHalo->haloid == thisnode->FofHalo->haloid)
                                    {
                                        ...
                                        if (float_almost_equal(minrsep, 0.0, 5) == 1 && (tag == 3 || tag == 5))
                                        {
                                            ...
                                            fprintf(stderr,
                                                    "thisnode->fofhaloid = %" STR_FMT " tmpnode->id = %" STR_FMT
                                        ...
                                        tmpmaxdelpotprim =
                                            get_internal_delpot_prim(tmpnode, tmpnode->FofHalo, minrsep, tmpvsep);
                                        MaxDelPotPrim =
                                            tmpmaxdelpotprim > MaxDelPotPrim ? tmpmaxdelpotprim : MaxDelPotPrim;

                                        tmpmaxdelpotsec =
                                            get_internal_delpot_sec(tmpnode, tmpnode->FofHalo, minrsep, tmpvsep);
                                    ...
                                    fprintf(fp,
                                            " %10d     %12" STR_FMT "   %12" STR_FMT "      %12.4g    %12" STR_FMT
                                            "   %12" STR_FMT "    %12.6g   %10.4f   %10.4f  %12.4g  %12.4g  "
                                            ...
                                            savenode->FofHalo->haloid, savenode->FofHalo->Mtot, thisnode->nodeloc,
                                            thisnode->haloid, thisnode->InfallMass, rsep, vsep, Impulse, DelPotPrim,
                                            DelPotSec, tag, destructionz, minrsep, MaxDelPotPrim, MaxDelPotSec,
                                            destructionradius, savenode->FofHalo->Rvir, minrsep_snap, rvir_at_minrsep,
                                            fof_rvir_at_minrsep, destruction_snapshot, destruction_subtmot,
                                            destruction_parentmtot, thisnode->Mstar, destruction_parentmstar);
                ...
                else
                {
                    ...
                    */
                    ...
                    if (thisnode->Nchild == 1)
                    {                        /*Nchild could be 0 as well at this section in the code ->
                        ...
                        if (thisnode->isFof == 0)
                        {
                            if (thisnode->FofHalo->haloid != thisnode->BigChild->FofHalo->haloid)
                            {
                                ...
                                if (tag == 3 || tag == 5)
                                {
                                    ...
                                    fprintf(fp2, "%e %e %e %e %e %e %e %e %e %" STR_FMT "  %" STR_FMT "\n", thisnode->z,
                                            thisnode->FofHalo->BoundFofMtot, thisnode->InfallMass,
                                            get_scalefactor(thisnode->z) *
                                       ...
                                       tmpnode->Parent->FofHalo->haloid == thisnode->FofHalo->haloid)
                                {
                                    ...
                                    if (float_almost_equal(minrsep, 0.0, 5) == 1 && (tag == 3 || tag == 5))
                                    {
                                        ...
                                        fprintf(stderr,
                                                "thisnode->haloid = %" STR_FMT " snapshot= %d  Fofid = %" STR_FMT " \n",
                                ...
                                fprintf(fp,
                                        " %10d     %12" STR_FMT "   %12" STR_FMT "      %12.4g    %12" STR_FMT
                                        "   %12" STR_FMT "    %12.6g   %10.4f   %10.4f  %12.4g  %12.4g  %12.4g  "
                ...
                if (thisnode->TotNmergers == 0)
                    ...
                    thisnode->TotNmergers += totnmergers; /* in case increment_fof_mergers has been called on
                                                             thisnode */
            }
        }
    }
...
void find_subsub_mergers(struct node_data *tree[], int64 *Ngroups)
{
    ...
    for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM; isnapshot >= PARAMS.MIN_SNAPSHOT_NUM; isnapshot--)
    {
        ...
        if (Ngroups[isnapshot] > 0)
        {
            ...
            {
                thisnode = &BaseNode[igroup];
                if (thisnode->isFof == 0 && thisnode->Parent != NULL && thisnode->Parent->haloid != thisnode->haloid &&
                    thisnode->Parent->isFof == 0 && thisnode->ContainerHalo->haloid != thisnode->Parent->haloid &&
                    thisnode->Parent->ContainerHalo->haloid == thisnode->ContainerHalo->haloid)
                {
                    ...
                    //  same containerhalo
                    fprintf(fp,
                            "%8hd   %14" STR_FMT " %14" STR_FMT "   %14" STR_FMT "          %8hd   %14" STR_FMT
                            "   %14" STR_FMT "     %14" STR_FMT "  \n",
                            thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->ContainerHalo->haloid,
                            thisnode->Parent->snapshot, thisnode->Parent->nodeloc, thisnode->Parent->haloid,
    ...
    while (thisnode != NULL && fofnode != NULL)
    {
        if (thisnode->isFof == 1 && thisnode->FofHalo->haloid != fofnode->FofHalo->haloid)
        {
            ...
            if (node1->BigChild != NULL && node1->BigChild->isFof == 1)
            {
                 ...
                 * negative */
                parentnode = node1->BigChild;  /* bad naming. parentnode here is actually in the
            ...
            *parentmstar = fofnode->Mstar;
            if (thisnode != NULL && thisnode->BigChild != NULL)
                *rnorm = get_separation_between_centres(thisnode->BigChild, thisnode->BigChild->FofHalo) /
                         thisnode->BigChild->FofHalo->Rvir; /* more like a separation radius*/
            else
                *rnorm = -1.0;
        ...
        if (thisnode->haloid != tag_haloid && thisnode->FofHalo->haloid == tag_fofhaloid)
        {
            ...
            {
                *z = tmpnode->z;
                *rnorm = get_separation_between_centres(tmpnode, thisnode->FofHalo->BigChild) /
                         thisnode->FofHalo->BigChild->Rvir; /* last known position of the subhalo*/
            }
...
void ensure_same_snapshot_for_halos(struct node_data **node1, struct node_data **node2)
{
    ...
    if ((*node1) != NULL && (*node2) != NULL)
    {
        ...
        while (1)
        {
            ...
            {
                fprintf(stderr, "in ensure_same_snapshot: failed to get to same snapshots. (at "
