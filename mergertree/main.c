        ...
        if (Ngroups[isnapshot] > 0)
        {
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (thisnode->Parent != NULL)
                    fprintf(fd,
                            "%6d  %14" STR_FMT " %14" STR_FMT "  %12d    %12d  %14" STR_FMT "   %14" STR_FMT "  %12d\n",
                            thisnode->snapshot, thisnode->haloid, thisnode->nodeloc, thisnode->isFof,
                            thisnode->Parent->snapshot, thisnode->Parent->haloid, thisnode->Parent->nodeloc,
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (thisnode->haloid == haloid)
                {
                    ...
                    fprintf(fp, "%14" STR_FMT "   %14d     %14.4g      %12d        %14.4g    %12d  %14.4g",
                            thisnode->haloid, isnapshot, thisnode->FormationRedshift, tmp->snapshot,
                            thisnode->DestructionRedshift, thisnode->snapshot, thisnode->Mtot);
                 ...
                 int isnapshot)
{
    ...
    for (int64 igroup = 0; igroup < Ngroups0; igroup++)
    {
        ...
        node[igroup].InfallMass = group0[igroup].Mtot; /* initialization -- will get the `real' value from
                                                          maketree later on*/
        ...
        if (isnapshot < PARAMS.MAX_SNAPSHOT_NUM && parent[igroup].parentsnapshot <= PARAMS.MAX_SNAPSHOT_NUM &&
            parent[igroup].parentsnapshot >= PARAMS.MIN_SNAPSHOT_NUM && parent[igroup].parentid >= 0)
        {
...
void writeids(struct node_data *tree[], int64 *Ngroups)
{
    ...
    for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM; isnapshot <= PARAMS.MAX_SNAPSHOT_NUM; isnapshot++)
    {
        ...
        for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
        {
            thisnode = &BaseNode[igroup];
            fprintf(fp,
                    "%4d     %12" STR_FMT "     %12" STR_FMT "    %13.4f    %12.3f      %12.3f    %10" STR_FMT
                    "    %10" STR_FMT "  %10d  %12" STR_FMT "  %12" STR_FMT "    %12" STR_FMT
                    "   %12.4g    %12.4g    %12.4g     %12" STR_FMT "    %12" STR_FMT "   %14.4lf   %14.4lf %14.6g \n",
                    isnapshot, igroup, thisnode->haloid, thisnode->Mtot, thisnode->RedshiftofLastMerger,
                    thisnode->FormationRedshift, thisnode->Nmergers, thisnode->TotNmergers, thisnode->ParentLevel,
                    thisnode->Nsub, thisnode->FofHalo->haloid, thisnode->ContainerId, thisnode->Rvir,
                    thisnode->Rvir_anyl, thisnode->Rhalf, thisnode->NFlybys, thisnode->TotNFlybys, thisnode->Mstar,
        ...
        if (Ngroups[isnapshot] > 0)
        {
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (childnode != NULL)
                {
                    ...
                    {
                        fprintf(fd,
                                "%4d  %14" STR_FMT "  %10" STR_FMT " %14d   %14" STR_FMT "   %14.4g  %14" STR_FMT
                                "  %14" STR_FMT "  %14d   %16.4g %14d\n",
                                isnapshot, igroup, BaseNode[igroup].haloid, BaseNode[igroup].ParentLevel,
                                BaseNode[igroup].Nsub, BaseNode[igroup].Mtot, childnode->nodeloc, childnode->haloid,
                                childnode->snapshot, childnode->Mtot, childnode->ParentLevel);
                ...
                else
                {
                    fprintf(fd,
                            ...
                            isnapshot, igroup, BaseNode[igroup].haloid, BaseNode[igroup].ParentLevel,
                            BaseNode[igroup].Nsub, BaseNode[igroup].Mtot, -1, -1, -1, -1.0, -1);
                }
            }
            fclose(fd);
        }
...
void output_mergers_for_haloid(struct node_data *tree[], int64 *Ngroups, const int64 haloid, const float mineta)
{
    ...
    else
    {
        ...
        fp = my_fopen(outfname, "w");
        my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR, "growth_history_for_haloid", haloid,
                    ".txt");
        ...
        while (bigchild->BigChild != NULL)
        {
            ...
            if (bigchild->Parent->Nchild > 1)
            {
                ...
                if (sibling->Mtot / bigchild->Mtot > mineta)
                    fprintf(fp,
                            ...
                            "ratio = %10.4f\n",
                            bigchild->z, bigchild->Mtot, sibling->Mtot, (sibling->Mtot / bigchild->Mtot));
            }

            if (bigchild->haloid == 0)
                fprintf(stderr,
                        "z = %7.2f  Nmergers = %4" STR_FMT "  TotNmergers = %6" STR_FMT "  NDisruptions = %4" STR_FMT
                        "  TotNdisrupt = %6" STR_FMT "  NDissolution = %4" STR_FMT "  TotNDissolutions = %6" STR_FMT
                        "\n",
            ...
            fprintf(fp1, "%10hd    %14" STR_FMT "   %14" STR_FMT "    %12.6g    %14" STR_FMT " \n", bigchild->snapshot,
                    bigchild->nodeloc, bigchild->haloid, bigchild->Mtot, bigchild->FofHalo->haloid);
        }
        fclose(fp);
        fclose(fp1);
    }
...
void output_mergers(struct node_data *tree[], int64 *Ngroups)
{
                    ...
                    if (bigchild->Sibling != NULL)
                    {
                        ...
                        do
                        {
                            ...
                            */
                            ...
                            fprintf(Fof,
                                    "%4d   %14" STR_FMT "   %14.5g   %10d   %8d      %8d    %14" STR_FMT
                                    "   %14" STR_FMT "   %15d   %15d  %14.5g   %14.5g     \n",
                                    isnapshot, node[igroup].haloid, node[igroup].Mtot, node[igroup].ParentLevel,
                                    bigchild->snapshot, thischild->snapshot, bigchild->haloid, thischild->haloid,
                                    bigchild->ParentLevel, thischild->ParentLevel, bigchild->Mtot, thischild->Mtot);
                            ...
                            if (printedbig == 0)
                            {
                                fprintf(Fofprop,
                                        ...
                                        bigchild->snapshot, node[igroup].haloid, bigchild->haloid, bigchild->nodeloc,
                                        bigchild->Mtot, bigchild->xcen, bigchild->ycen, bigchild->zcen);
                                printedbig = 1;
                            }
                            ...
                            fprintf(Fofprop,
                                    ...
                                    thischild->snapshot, node[igroup].haloid, thischild->haloid, thischild->nodeloc,
                                    thischild->Mtot, thischild->xcen, thischild->ycen, thischild->zcen);
                    ...
                    if (node[igroup].Nchild == 1)
                    {
                        ...
                        */
                        ...
                        fprintf(Isolated, "%4d   %14" STR_FMT "    %14" STR_FMT "   %14.4g \n", isnapshot,
                                node[igroup].haloid, node[igroup].nodeloc, node[igroup].Mtot);
                    }
                }
            }
        }
    ...
    fprintf(Fof, "########################################################################"
                 "##############################################################\n");
    fprintf(Fof, "# Snap        ParentHaloid     ParentMtot    BigChSnap   OtherChSnap    "
                        ...
                        do
                        {
                            ...
                            if (thischild->Mtot / bigchild->Mtot >= MinMergerRatio)
                            {
                                ...
                                */
                                ...
                                fprintf(Fof,
                                        "%4d   %14" STR_FMT "   %14.4f   %8d      %8d    %14" STR_FMT "   %14" STR_FMT
                                        "    %14.4f   %14.4f     \n",
                                        isnapshot, node[igroup].haloid, node[igroup].Mtot, bigchild->snapshot,
                                        thischild->snapshot, bigchild->haloid, thischild->haloid, bigchild->Mtot,
                                        thischild->Mtot);
                            }
                        } while (thischild->Sibling != NULL);
                    }
                ...
                if (node[igroup].BigChild != NULL)
                {
                        ...
                        (thisfofhalo->haloid != node[igroup].haloid))
                    {
                        if (node[igroup].Mtot / thisfofhalo->Mtot >= MinMergerRatio)
                        {
                            ...
                            fprintf(Fof,
                                    "%4d   %14" STR_FMT "   %14.4f   %14" STR_FMT "    %14.4f    %12" STR_FMT
                                    "  %12d\n",
                                    isnapshot, thisfofhalo->haloid, thisfofhalo->Mtot, node[igroup].haloid,
                                    node[igroup].Mtot, bigchildfofhalo->haloid, bigchildfofhalo->snapshot);
                        }
                    }
                }
            }
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (thisnode->isFof == 1)
                {
                    ...
                    fprintf(fp, "%10d  %12" STR_FMT "   %12.4g   %10" STR_FMT "      %10" STR_FMT "%12.4f \n",
                            isnapshot, thisnode->haloid, thisnode->Mtot, totnmergers, totnflybys,
                    ...
                    if (container->Parent != NULL && thisnode->Parent != NULL)
                    {
                        if (container->Parent->haloid != container->haloid &&
                            ...
                            fprintf(fp, "%10d  %12" STR_FMT "   %12.4g    %10d    %12" STR_FMT "    %12.4f   %12.4f\n",
                                    isnapshot, thisnode->haloid, thisnode->Mtot, thisnode->ParentLevel,
                                    container->haloid, thisnode->FormationRedshift, container->FormationRedshift);
                    }
                }
            }
        }
                ...
                if (thisnode->Nsub > 1)
                {
                    ...
                    if ((thisnode->Mtot > Mthresh) && bigsub->Mtot / thisnode->Mtot > Mratio)
                        fprintf(fp, "%10d  %12" STR_FMT "   %12.4g     %12" STR_FMT "   %12.4g    %12.4f   %12.4f\n",
                                isnapshot, thisnode->haloid, thisnode->Mtot, bigsub->haloid, bigsub->Mtot,
                                thisnode->FormationRedshift, bigsub->FormationRedshift);
                }
                ifof += thisnode->Nsub;
            }
        }
                    ...
                    while (thisnode != NULL && thisnode->BigChild != NULL)
                    {
                        ...
                        if (thisnode->BigChild->Mtot / thisnode->Mtot < Mratio && thisnode->BigChild->Mtot > Mthresh)
                            fprintf(fp, "%6d   %12" STR_FMT " %12" STR_FMT "  %12.4g   %10d  %12" STR_FMT "  %13.4g \n",
                                    ...
                                    thisnode->BigChild->haloid, thisnode->BigChild->Mtot, thisnode->snapshot,
                                    thisnode->nodeloc, thisnode->Mtot);
                        ...
                        if (thisnode->BigChild->Mtot / thisnode->Mtot > (1.0 / Mratio) && thisnode->Mtot > Mthresh)
                            fprintf(
                                fp1, "%6d   %12" STR_FMT " %12" STR_FMT "  %12.4g   %10d   %12" STR_FMT " %13.4g \n",
    ...
    for (isnapshot = PARAMS.MAX_SNAPSHOT_NUM; isnapshot >= PARAMS.MIN_SNAPSHOT_NUM; isnapshot--)
    {
        if (Ngroups[isnapshot] > 0)
        {
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                {
                    fprintf(fp,
                            "%10d   %14" STR_FMT "   %14" STR_FMT "   %12.6g      %14" STR_FMT
                            "   %12.4g    %12" STR_FMT " \n",
                            thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->Mtot,
                            thisnode->FofHalo->haloid, thisnode->FofHalo->Mtot, thisnode->FofHalo->Nsub);
                }

                if (thisnode->Parent != NULL && thisnode->Parent->haloid != thisnode->haloid &&
                    thisnode->Mtot > Mthresh)
                {
                    fprintf(fp1,
                            "%10d   %14" STR_FMT "   %14" STR_FMT "   %12.4g    %12hd    %14" STR_FMT "    %12" STR_FMT
                            "  %12.6g   \n",
                            thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->Mtot,
                            thisnode->ParentLevel, thisnode->ContainerHalo->haloid, thisnode->Parent->haloid,
                ...
                if (thisnode->Parent == NULL && thisnode->Mtot > Mthresh)
                {
                    fprintf(fp2, "%10d   %14" STR_FMT "   %14" STR_FMT "   %12.4g   \n", thisnode->snapshot,
                            thisnode->nodeloc, thisnode->haloid, thisnode->Mtot);
                }
            }
        }
    }
...
void print_flyby_future_header(FILE *fp)
{
    ...
    fprintf(fp, "####################################################################"
                "####################################################################"
                "#############################################################\n");
    fprintf(fp, "#    snapshot       redshift              haloid       groupnum     "
    ...
    for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM; isnapshot > PARAMS.MAX_SNAPSHOT_NUM - 2; isnapshot--)
    {
        if (Ngroups[isnapshot] > 0)
        {
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (mwnode->isFof == 1 && mwnode->Mtot >= Mmin && mwnode->Mtot <= Mmax &&
                    mwnode->FormationRedshift > MinFormZ)
                {
                    ...
                    {
                        mwnode = &BaseNode[igroup];
                        my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR, "MW_flybys",
                                    mwnode->haloid, ".txt");
                    ...
                    if (flag != 1)
                    {
                        ...
                        while (mwnode != NULL)
                        {
                            ...
                            while (thisnode != NULL && thisnode->isFof == 0 &&
                                   thisnode->nodeloc < (Ngroups[thisnode->snapshot] - 1) && thisnode->FofHalo == mwnode)
                            {
                                /* 							  tag =
                                ...
                                tag =
                                    get_tag(thisnode, mwnode, &destructionz, &destructionradius, &destruction_snapshot,
                                            &destruction_subtmot, &destruction_parentmtot, &destruction_mstar);
                                if (tag == 3 || tag == 5)
                                { /* flyby tags -> should make it into a function
                                    ...
                                    fprintf(fp,
                                            ...
                                            mwnode->snapshot, mwnode->z, mwnode->haloid, mwnode->nodeloc, mwnode->Mtot,
                                            mwnode->BoundFofMtot, thisnode->haloid, thisnode->nodeloc, thisnode->Mtot,
...
void output_milkyway_halos(struct node_data *tree[], int64 *Ngroups)
{
    ...
    for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM; isnapshot > PARAMS.MAX_SNAPSHOT_NUM - 1; isnapshot--)
    {
        if (Ngroups[isnapshot] > 0)
        {
            ...
            {
                thisnode = &node[igroup];
                if (thisnode->ParentLevel == 1 && thisnode->Mtot >= Mmin && thisnode->Mtot <= Mmax &&
                    thisnode->FormationRedshift > MinFormZ)
                {
                    /* All right, so we have a MW type halo at z ~ 0 */
                    flag = 0;
                    fof_mwcount++;
                    ...
                    thisnode = &node[igroup];
                    my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR, "MW", thisnode->haloid,
                                ".txt");
                    ...
                    if (flag != 1)
                    {
                        ...
                        while (thisnode != NULL)
                        {
                            ...
                            fprintf(fp,
                                    " %6d    %14" STR_FMT "    %14" STR_FMT "   %14.4g    %10d     %14" STR_FMT
                                    "     %10" STR_FMT "   %10d   %14u  \n",
                                    thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->Mtot,
                                    thisnode->ParentLevel, thisnode->FofHalo->haloid, parentid, parentsnap,
                        ...
                        thisnode = &node[igroup];
                        mwnode = &node[igroup];
                        my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR, "MW_allsubhalos",
                                    thisnode->haloid, ".txt");
                        fp = my_fopen(outfname, "w");
                        fprintf(fp,
                                "# MW type halo. Haloid = %" STR_FMT
                        ...
                        while (mwnode != NULL)
                        {
                            ...
                            for (int64 isub = mwnode->nodeloc; isub < (mwnode->nodeloc + mwnode->Nsub); isub++)
                            {
                                ...
                                fprintf(
                                    ...
                                    "   %14.4g    %10d   %14.4g   %14u    %14" STR_FMT "     %10d \n",
                                    thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->Mtot,
                                    thisnode->ParentLevel,
    ...
    for (int64 igroup = 0; igroup < Ngroups[snapshot]; igroup++)
    {
        ...
        if (clusternode->isFof == 1 && clusternode->BoundFofMtot >= Mmin && halo_is_always_fof(clusternode) == 1)
        {
                        ...
                        if (thisnode != NULL && thisnode->FofHalo->haloid == clusternode->haloid)
                        {
                            ...
                            if (thisnode != NULL && thisnode->FofHalo->haloid == clusternode->haloid)
                            {
                                ...
                                tag = get_tag(thisnode, thisnode->FofHalo, &destructionz, &destructionradius,
                                              &destruction_snapshot, &destruction_subtmot, &destruction_parentmtot,
                                              &destruction_mstar);
                            }
            ...
            for (int64 isub = 0; isub < Nsubhaloids; isub++)
            {
                ...
                // new scope
                {
                    ...
                    if (tmp_fof->haloid != clusternode->haloid)
                    {
                        fprintf(stderr,
                                ...
                                "with haloid = %" STR_FMT " instead I have haloid = %" STR_FMT "..exiting\n",
                    ...
                    // assigned.)
                    while (subnode != NULL && subnode->haloid == subhaloids[isub] && tmp_fof != NULL &&
                           tmp_fof->haloid == clusternode->haloid)
                    {
                ...
                subnode = subhalos[isub];
                fprintf(fp,
                        "%14" STR_FMT " %14" STR_FMT "  %14.6f     %14.6f  %14.6f  %14.6f   %14.6f   %14.6f  %10d   "
                        ...
                        subnode->Rvir, clusternode->Rvir, dmin, rvir1_at_min, rvir2_at_min, snapshot_for_dmin,
                        subnode->InfallMass, subnode->Mtot, alltags[isub], dmax, rvir1_at_max, rvir2_at_max,
                        snapshot_for_dmax);
