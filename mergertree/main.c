...
NDissolution -> Different FOF parents between subhalos/Fof halos that now reside
            in one FOF halo e.g., one FOF halo comes in and gets destroyed
inside a different FOF halo.


NMergers     -> A subhalo/FOF halo comes inside a FOF halo and *RETAINS* its
identity. This halo will continue to exist for multiple snapshots and get
tidally stripped until it reaches the point where it disappears.


NDisruptions -> A subhalo disappears inside a FOF halo as a result of tidal
stripping etc. A merger counted in Nmerger is complete when a corresponding
entry is seen in NDisruptions.
...
#error Please select only one of the cosmologies from various WMAP options in the Makefile
  ...
  int64 Ngroups[NUM_SNAPSHOTS]; // there are groups corresponding to
                                // MAX_SNAPSHOT_NUM
  struct parent_data
      *allparents[PARAMS.MAX_SNAPSHOT_NUM]; // parents_??? files only go up to
                                            // MAX_SNAPSHOT_NUM-1
  struct node_data *tree[NUM_SNAPSHOTS]; // there are groups in MAX_SNAPSHOT_NUM

  // output the parameter file
  my_snprintf(outfname, MAXLEN, "%s/complete_mergertree.params",
              PARAMS.OUTPUT_DIR);
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
       isnapshot <= PARAMS.MAX_SNAPSHOT_NUM; isnapshot++)
    Ngroups[isnapshot] = 0;
...
#define RETURN_ONLY_FOFS 0
    ...
    my_snprintf(outfname, MAXLEN, "%s/%s%05d.z%5.3f.AHF_halos",
                PARAMS.GROUP_DIR, PARAMS.GROUP_BASE, isnapshot,
                REDSHIFT[isnapshot]);
...
#undef RETURN_ONLY_FOFS
    ...
    if (Ngroups0 > 0) {
      ...
      fprintf(stderr,
              "loading group for snapshot # %d with %" STR_FMT " halos ",
              isnapshot, Ngroups0);
      ...
      t_sectionstart = time(NULL);
      my_snprintf(outfname, MAXLEN, "%s/subhalolevel_%03d.txt",
                  PARAMS.OUTPUT_DIR, isnapshot);
...
#if ((defined(GET_GROUPVEL) == 0) && (defined(GET_MEANVEL) == 0))
      my_snprintf(snapshotname, MAXLEN, "%s/%s_%03d", PARAMS.SNAPSHOT_DIR,
                  PARAMS.SNAPSHOT_BASE, isnapshot);
      t_sectionstart = time(NULL);
    ...
    my_snprintf(
        outfname, MAXLEN, "%s %d ",
        "Reading groups and assigning to node for i = ",
  ...
  /* output the translation table from group number to group id at all
   * snapshots*/
  t_sectionstart = time(NULL);
  ...
  print_time(t_sectionstart, t_sectionend,
             "output cumulative mergers for all snapshots");

  t_sectionstart = time(NULL);
  output_interesting_halos(tree, Ngroups);
  t_sectionend = time(NULL);
  print_time(t_sectionstart, t_sectionend,
             "output haloids for `interesting' halos");

  t_sectionstart = time(NULL);
  output_suspicious_halos(tree, Ngroups);
  t_sectionend = time(NULL);
  print_time(t_sectionstart, t_sectionend,
             "output haloids for `suspicious' halos");

  t_sectionstart = time(NULL);
  output_milkyway_halos(tree, Ngroups);
  t_sectionend = time(NULL);
  print_time(t_sectionstart, t_sectionend,
             "output all MW type halos at z = 0.0");
...

  {
    if (PARAMS.INDIVIDUAL_MERGERS == 1) {
      int64 output_haloid;
      fprintf(stderr,
              "Do you want to output mergers for a specific haloid (y/n) ? \n");
      scanf("%s", answer);
      while (answer[0] == 'y') {
        ...
        fprintf(stderr,
                "Do you want to output mergers for another haloid (y/n) ? \n");
        scanf("%s", answer);
      }
    }
  }
...
void print_makefile_options() {
...
#else
  fprintf(stderr, "The code was compiled to *NOT* read in velocities from the "
                  "group files \n");
#endif
  ...
  fprintf(stderr, "The code was compiled to track black holes by the centres "
                  "of halos they were assigned to \n");
#endif
...
void readsubhalo_hierarchy_levels(const char *fname,
                                  struct group_data *group0) {
  FILE *fp = NULL;
  ...
  while (1) {
    if (fgets(str_line, MAXLINESIZE, fp) != NULL) {
      if (str_line[0] != comment) {
         ...
         * %"STR_FMT"  %"STR_FMT"     %hd",  */
        sscanf(str_line,
               "%*d  %" STR_FMT " %hd   %" STR_FMT "  %" STR_FMT "     %hd",
               &dummy, &(group0[i].ParentLevel), &(group0[i].ContainerIndex),
               &(group0[i].Nsub),
        ...
        if (dummy != i) {
          fprintf(stderr,
                  ...
                  fname);
          fprintf(stderr,
                  "expected igroup = %" STR_FMT " instead got %" STR_FMT
                  " ..exiting \n\n",
                  i, dummy);
          exit(EXIT_FAILURE);
        }
...
void output_parents(struct node_data *tree[], int64 *Ngroups) {
  ...
  init_my_progressbar(PARAMS.MAX_SNAPSHOT_NUM - PARAMS.MIN_SNAPSHOT_NUM + 1,
                      &interrupted);
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
    ...
    if (Ngroups[isnapshot] > 0) {
      ...
      my_snprintf(outfname, MAXLEN, "%s/parentlist_%03d.txt", PARAMS.OUTPUT_DIR,
                  isnapshot);
      fd = my_fopen(outfname, "w");
      fprintf(fd, "############################################################"
                  "########################################################\n");
      fprintf(fd, "# Snapshot      Haloid         GroupNum      isFOF        "
                  "ParentSnap    ParentId        ParentNum      ParentisFOF\n");
      fprintf(fd, "#    i            l               l              i          "
                  "    i              l               l              i     \n");
      fprintf(fd, "############################################################"
                  "########################################################\n");
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (thisnode->Parent != NULL)
          fprintf(fd,
                  "%6d  %14" STR_FMT " %14" STR_FMT
                  "  %12d    %12d  %14" STR_FMT "   %14" STR_FMT "  %12d\n",
                  thisnode->snapshot, thisnode->haloid, thisnode->nodeloc,
                  thisnode->isFof, thisnode->Parent->snapshot,
                  thisnode->Parent->haloid, thisnode->Parent->nodeloc,
                  thisnode->Parent->isFof);
        else
          fprintf(fd,
                  "%6d  %14" STR_FMT " %14" STR_FMT
                  "  %12d    %12d  %14d   %14d  %12d\n",
                  thisnode->snapshot, thisnode->haloid, thisnode->nodeloc,
                  thisnode->isFof, -1, -1, -1, -1);
      }
      fclose(fd);
    }
  }
...
void output_all_haloids(struct node_data *tree[], int64 *Ngroups) {
  FILE *fp = NULL;
  char fname[MAXLEN];
  struct node_data *BaseNode = NULL, *thisnode = NULL, *tmp = NULL,
  ...
  fprintf(fp, "################################################################"
              "################################################################"
              "######################################\n");
  fprintf(fp, "#     Haloid      HaloIDAssignedSnap     FormationZ        "
              "FormationSnapshot     DestructionZ    DestructionSnap  HaloMass "
              "    EarliestSnap_as_Subhalo    MaxFOFMass  \n");
  fprintf(fp, "#        l              i                    f                  "
              " i                   f                 i               d        "
              "         i                   d        \n");
  fprintf(fp, "################################################################"
  ...
  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
       isnapshot >= PARAMS.MIN_SNAPSHOT_NUM; isnapshot--) {
    if (Ngroups[isnapshot] > 0) {
      BaseNode = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        first_as_subhalo = NULL;
        ...
        if (thisnode->haloid == haloid) {
          ...
          fprintf(fp,
                  "%14" STR_FMT
                  "   %14d     %14.4g      %12d        %14.4g    %12d  %14.4g",
                  thisnode->haloid, isnapshot, thisnode->FormationRedshift,
                  tmp->snapshot, thisnode->DestructionRedshift,
...
void assign_vxcm(struct group_data *group, int64 Ngroups,
  ...
  for (i = 0; i < Ngroups; i++) {
    ...
    for (j = 0; j < group[i].N; j++) {
      ...
      summ += mass;
#ifndef FOF_ONLY
      sumx += (periodic(P[index].Pos[0] - group[i].xcen) *
               mass); /* computing the centre of mass -> needs to account for
                         box wrapping. */
      sumy += (periodic(P[index].Pos[1] - group[i].ycen) * mass);
      sumz += (periodic(P[index].Pos[2] - group[i].zcen) * mass);
#else
      sumx += (periodic(P[index].Pos[0] - group[i].xcen) *
               mass); /* Make sure that group[i].xcen is initiliased by
                         loadgroups under the FOF_ONLY flag. */
      sumy += (periodic(P[index].Pos[1] - group[i].ycen) * mass);
      sumz += (periodic(P[index].Pos[2] - group[i].zcen) * mass);
#endif
...
void assign_node(struct group_data *group0, int64 Ngroups0,
  ...
  my_snprintf(fname, MAXLEN, "%s/allgroupvels_%03d.txt", PARAMS.GROUP_DIR,
              isnapshot);
  ...
  double rhocrit;
  float overdensity; /*Warning: Make sure that the definitions dont confuse
                        between \rho_u and \rho_{crit}. Overdensity of 200 is
                        usually defined wrt \rho_{crit} */
  ...
  rhocrit *= (scale_factor * scale_factor *
              scale_factor); /* in co-moving co-ordinates */
  ...
  for (int64 igroup = 0; igroup < Ngroups0; igroup++) {
    ...
    /* 		{ */
    /* 		  /\* this will assign the sum of all the subhalo masses as the bound
     * fof mtot. however, this */
    /* 			 will miss the last fof halo if it has only itself as the bound
     * subhalo -- */
    ...
    if (group0[igroup].isFof == 1) {
      ...
      FOF_Parent->BoundFofMtot += group0[igroup].Mtot;
      node[igroup].BoundFofMtot =
          group0[igroup].Mtot; /* Not really proper -- subhalos have their
...
#ifndef GET_MEANVEL
    node[igroup].vxcen =
        group0[igroup].vxcen; /* group vels have been loaded in from snapshots.
    ...
    node[igroup].InfallMass =
        group0[igroup].Mtot; /* initialization -- will get the `real' value from
                                maketree later on*/
    ...
    node[igroup].Rvir_anyl =
        getrvir_anyl(group0[igroup].Mtot, REDSHIFT[isnapshot], PARAMS.COSMO);
    getrvir_from_overdensity(
        &group0[igroup], Nbins, rhocrit,
     ...
     * Rvir_anyl=%lf  Rhalf = %lf Maxoverdensity=%e  group->N = %"STR_FMT"
    ...
    if (group0[igroup].Rvir > 0.0) {
      ...
      node[igroup].Rhalf =
          node[igroup].Rvir_anyl * 0.5; /* hack -> half mass radius = 1/2 Rvir.
                                           Better fit in Lokas 2001  */
      node[igroup].Rvir = node[igroup].Rvir_anyl;
      node[igroup].Conc = 0.0; /* use concentration and mass to fit
    ...
    // The parent structure will not exist for snapshot PARAMS.MAX_SNAPSHOT_NUM.
    // initialize from
    //  the group directory.
    ...
    if ((node[igroup].ContainerHalo)->Nsub == 0) {
      fprintf(stderr,
              ...
              "Nsubs. snapshot = %hd igroup = %" STR_FMT
              " Container id = %" STR_FMT " ..exiting\n",
              isnapshot, igroup, node[igroup].ContainerId);
      exit(EXIT_FAILURE);
    }

    /* group0 is going to be freed once this function returns. No need to fill
     * it up. */
    /* 	  group0[igroup].ContainerId   = parent[igroup].containerid; */
    /* 	  group0[igroup].ParentLevel   = parent[igroup].parentlevel; */

    if (isnapshot < PARAMS.MAX_SNAPSHOT_NUM &&
        parent[igroup].parentsnapshot <= PARAMS.MAX_SNAPSHOT_NUM &&
        parent[igroup].parentsnapshot >= PARAMS.MIN_SNAPSHOT_NUM &&
        parent[igroup].parentid >= 0) {
      /* 			  fprintf(stderr,"isnapshot = %d igroup = %d
       * parent snapshot =
    ...
    sscanf(str_line, "%" STR_FMT " %f  %f  %f\n", &tmp_grpnum,
           &(node[igroup].meanvel[0]), &(node[igroup].meanvel[1]),
           &(node[igroup].meanvel[2]));
    if (tmp_grpnum != igroup) {
      fprintf(stderr, "Error: While reading in velocites for the groups\n");
      fprintf(stderr,
              "expected groupnum = %" STR_FMT "  got groupnum = %" STR_FMT
              " ..exiting\n",
              igroup, tmp_grpnum);
      exit(EXIT_FAILURE);
    }
    ...
    node[igroup].Seeded =
        0; /* This halo has not 'yet' been seeded with a black hole */
  }
#ifdef GET_MEANVEL
  fclose(fp);
#endif
...
void writeids(struct node_data *tree[], int64 *Ngroups) {
  ...
  /* PRINTSTEP =
   * (int)floor(0.1*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM)); */
  /* SMALLPRINTSTEP =
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM)) > 1 ?
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM)):1; */
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
    ...
    BaseNode = tree[isnapshot];
    my_snprintf(outfname, MAXLEN, "%s/table_%03d.txt", PARAMS.OUTPUT_DIR,
                isnapshot);
    ...
    fprintf(fp, "# Mass is in %e Msun. Redshift = %f  \n", ActualMassUnits,
            REDSHIFT[isnapshot]);
    fprintf(fp, "#  \n");
    fprintf(fp, "# Snapshot        GroupNum        Haloid           Mtot       "
                "  LastMergerRed     Formationz      Nmergers    TotNmergers   "
                "  ParentLev       Nsub     FofHaloid    ContainerNum       "
                "Rvir          Rvir_anyl         Rhalf       Nflybys       "
                "TotNflybys     Mstar    InfallMass       xcen \n");
    fprintf(fp, "#   i                l               l               d        "
                "         f               f               l           l        "
                "      i              l         l             l                "
                "d             d                 d           l               l "
    ...
    for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
      thisnode = &BaseNode[igroup];
      fprintf(fp,
              "%4d     %12" STR_FMT "     %12" STR_FMT
              "    %13.4f    %12.3f      %12.3f    %10" STR_FMT
              "    %10" STR_FMT "  %10d  %12" STR_FMT "  %12" STR_FMT
              "    %12" STR_FMT "   %12.4g    %12.4g    %12.4g     %12" STR_FMT
              "    %12" STR_FMT "   %14.4lf   %14.4lf %14.6g \n",
              isnapshot, igroup, thisnode->haloid, thisnode->Mtot,
              thisnode->RedshiftofLastMerger, thisnode->FormationRedshift,
              thisnode->Nmergers, thisnode->TotNmergers, thisnode->ParentLevel,
              thisnode->Nsub, thisnode->FofHalo->haloid, thisnode->ContainerId,
              thisnode->Rvir, thisnode->Rvir_anyl, thisnode->Rhalf,
              thisnode->NFlybys, thisnode->TotNFlybys, thisnode->Mstar,
...
void output_children(struct node_data *tree[], int64 *Ngroups) {
   ...
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM)) > 1 ?
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM)):1; */
  init_my_progressbar(PARAMS.MAX_SNAPSHOT_NUM - PARAMS.MIN_SNAPSHOT_NUM,
                      &interrupted);
  ...
  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
    ...
    if (Ngroups[isnapshot] > 0) {
      BaseNode = tree[isnapshot];
      my_snprintf(outfname, MAXLEN, "%s/children_%03d.txt", PARAMS.OUTPUT_DIR,
                  isnapshot);
      ...
      fprintf(fd, "############################################################"
                  "############################################################"
                  "###########################################\n");
      fprintf(fd, "# Snapshot     GroupNum     HaloId         HierarLevel    "
                  "Nsub          Mtot            ChildNum         ChildId      "
                  "  ChildSnap         Mchild   ChildHierarlevel\n");
      fprintf(fd, "#    i            l           l                i            "
                  "l            d                 l                l           "
                  "   i                 d             i       \n");
      fprintf(fd, "############################################################"
      ...
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (childnode != NULL) {
          while (childnode != NULL) {
            fprintf(fd,
                    "%4d  %14" STR_FMT "  %10" STR_FMT " %14d   %14" STR_FMT
                    "   %14.4g  %14" STR_FMT "  %14" STR_FMT
                    "  %14d   %16.4g %14d\n",
                    isnapshot, igroup, BaseNode[igroup].haloid,
                    BaseNode[igroup].ParentLevel, BaseNode[igroup].Nsub,
                    BaseNode[igroup].Mtot, childnode->nodeloc,
                    childnode->haloid, childnode->snapshot, childnode->Mtot,
                    childnode->ParentLevel);
            childnode = childnode->Sibling;
          }
        } else {
          fprintf(fd,
                  "%4d  %14" STR_FMT "  %10" STR_FMT " %14d   %14" STR_FMT
                  "   %14.4g  %14d  %14d  %14d   %16.4g  %14d\n",
                  isnapshot, igroup, BaseNode[igroup].haloid,
                  BaseNode[igroup].ParentLevel, BaseNode[igroup].Nsub,
...
void output_mergers_for_haloid(struct node_data *tree[], int64 *Ngroups,
  ...
  for (isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
       isnapshot >= PARAMS.MIN_SNAPSHOT_NUM; isnapshot--) {
    if (Ngroups[isnapshot] > 0) {
      igroup = 0;
      node = tree[isnapshot];
      ...
      /* Assumes that once Ngroups is zero for some snapshot, it is zero for all
         previous snapshots. If your simulation does not have this 'usual'
         behaviour and you really want the history for the disappearing halos,
         comment this else section. */
      igroup =
          Ngroups[isnapshot] + 1; /* to satisfy the following if condition*/
      break;
    }
  }
  ...
  if (igroup >= Ngroups[isnapshot]) {
    ...
    my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR,
                "mergers_for_haloid", haloid, ".txt");
    fp = my_fopen(outfname, "w");
    my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s", PARAMS.OUTPUT_DIR,
                "growth_history_for_haloid", haloid, ".txt");
    fp1 = my_fopen(outfname, "w");
    fprintf(fp1, "#############################################################"
                 "##################\n");
    fprintf(fp1, "#   snapshot         groupnum          haloid          mtot  "
                 "       Fofhaloid #\n");
    fprintf(fp1, "#############################################################"
                 "##################\n");
    ...
    while (bigchild->BigChild != NULL) {
      ...
      if (bigchild->Parent->Nchild > 1) {
        ...
        if (sibling->Mtot / bigchild->Mtot > mineta)
          fprintf(fp,
                  " At z = %7.2f  primary mass = %12.4g other mass = %12.4g  "
                  "ratio = %10.4f\n",
                  bigchild->z, bigchild->Mtot, sibling->Mtot,
                  (sibling->Mtot / bigchild->Mtot));
      }

      if (bigchild->haloid == 0)
        fprintf(stderr,
                "z = %7.2f  Nmergers = %4" STR_FMT "  TotNmergers = %6" STR_FMT
                "  NDisruptions = %4" STR_FMT "  TotNdisrupt = %6" STR_FMT
                "  NDissolution = %4" STR_FMT "  TotNDissolutions = %6" STR_FMT
                "\n",
                bigchild->z, bigchild->Nmergers, bigchild->TotNmergers,
                bigchild->NDisruptions, bigchild->TotNDisruptions,
                bigchild->NDissolutions, bigchild->TotNDissolutions);

      fprintf(fp1,
              "%10hd    %14" STR_FMT "   %14" STR_FMT
              "    %12.6g    %14" STR_FMT " \n",
              bigchild->snapshot, bigchild->nodeloc, bigchild->haloid,
              bigchild->Mtot, bigchild->FofHalo->haloid);
    }
    fclose(fp);
    fclose(fp1);
  }
...
void output_mergers(struct node_data *tree[], int64 *Ngroups) {
  ...
  fprintf(Fof,
          ...
          "###################################################\n");
  fprintf(Fof,
          "# Snap        ParentHaloid     ParentMtot    ParentHLevel      "
          "BigChSnap     OtherChSnap        BigChId        OtherChId        "
          "BigChHLevel     OtherChHlevel   BigChMtot     OtherChMtot  \n");
  fprintf(Fof,
          "#  i              l                d             i                i "
          "               i               l                l               i   "
          "              i             d              d       \n");
  ...
  fprintf(Fofprop, "###########################################################"
                   "######################################################\n");
  fprintf(Fofprop, "# HaloSnap      ParentId        HaloId      HaloLoc       "
                   "HaloMass         xcen            ycen            zcen  \n");
  fprintf(Fofprop, "#    i             l               l           l           "
                   "  d               f              f                f   \n");
  fprintf(Fofprop, "###########################################################"
                   "######################################################\n");
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
    ...
    my_progressbar(isnapshot - PARAMS.MIN_SNAPSHOT_NUM, &interrupted);
    if (Ngroups[isnapshot] > 0) {
      /* 		  fprintf(stderr,"\n\n Now writing out all mergers at
       * snapshot %d Ngroups = %"STR_FMT" \n",isnapshot,Ngroups[isnapshot]); */
      node = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        if (node[igroup].Nchild > 1) {
          /* 			  fprintf(stderr,"for igroup = %d nchild =
          ...
          if (bigchild->Sibling != NULL) {
            ...
            do {
              ...
              /*
                 There may be a situation where the big child is not from the
                 previous snapshot but all the rest of the children are. There
                 needs to be a check for that.
              */

              fprintf(Fof,
                      "%4d   %14" STR_FMT
                      "   %14.5g   %10d   %8d      %8d    %14" STR_FMT
                      "   %14" STR_FMT "   %15d   %15d  %14.5g   %14.5g     \n",
                      isnapshot, node[igroup].haloid, node[igroup].Mtot,
                      node[igroup].ParentLevel, bigchild->snapshot,
                      thischild->snapshot, bigchild->haloid, thischild->haloid,
                      bigchild->ParentLevel, thischild->ParentLevel,
                      bigchild->Mtot, thischild->Mtot);

              if (printedbig == 0) {
                fprintf(Fofprop,
                        "%4d  %14" STR_FMT "   %14" STR_FMT "   %8" STR_FMT
                        "   %14.5lf   %14.5lf  %14.5lf  %14.5lf  \n",
                        bigchild->snapshot, node[igroup].haloid,
                        bigchild->haloid, bigchild->nodeloc, bigchild->Mtot,
                        bigchild->xcen, bigchild->ycen, bigchild->zcen);
                printedbig = 1;
              }
              ...
              fprintf(Fofprop,
                      ...
                      thischild->snapshot, node[igroup].haloid,
                      thischild->haloid, thischild->nodeloc, thischild->Mtot,
                      thischild->xcen, thischild->ycen, thischild->zcen);

            } while (thischild->Sibling != NULL);
          } else {
            fprintf(stderr, "\nError: The parent claims to have more than one "
                            "child but I can only find one\n");
            fprintf(stderr,
                    "Snapshot = %d  group num = %" STR_FMT
                    "  big child = %" STR_FMT " \n",
          ...
          if (node[igroup].Nchild == 1) {
            /*
               this is where I write out the haloes that are not doing anything
               ...as in possible isolated halos. Not really required since a
               combination of parents_xxx.txt, mergers.txt and table_xxx.txt
               will tell you which halos did not have a merger.
            */

            fprintf(Isolated,
                    "%4d   %14" STR_FMT "    %14" STR_FMT "   %14.4g \n",
                    isnapshot, node[igroup].haloid, node[igroup].nodeloc,
                    node[igroup].Mtot);
          }
        }
      }
  ...
  /*Here on I just repeat the previous steps -- and produce mergers bigger than
   * some ratio*/

  my_snprintf(outfname, MAXLEN, "%s/%s", PARAMS.OUTPUT_DIR,
              "bigcompletedmergers.txt");
  ...
  fprintf(
      Fof,
      "# Snap        ParentHaloid     ParentMtot    BigChSnap   OtherChSnap    "
      "    BigChId        OtherChId        BigChMtot     OtherChMtot \n");
  fprintf(
      Fof,
      "#   i              l               d             i           i          "
      "       l              l                d              d       \n");
  ...
  fprintf(stderr, "\n\nwriting to '%s'\n", outfname);
  /*   for(isnapshot=PARAMS.MAX_SNAPSHOT_NUM-1;isnapshot >=
   * PARAMS.MIN_SNAPSHOT_NUM;isnapshot--) */
  init_my_progressbar(PARAMS.MAX_SNAPSHOT_NUM - PARAMS.MIN_SNAPSHOT_NUM + 1,
                      &interrupted);
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
       isnapshot <= PARAMS.MAX_SNAPSHOT_NUM; isnapshot++) {
    my_progressbar(isnapshot - PARAMS.MIN_SNAPSHOT_NUM, &interrupted);
    if (Ngroups[isnapshot] > 0) {
      /* 		  fprintf(stderr,"\n Now writing out all mergers bigger
       * than %7.2f at snapshot %d \n",MinMergerRatio,isnapshot); */
      node = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        if (node[igroup].Nchild > 1) {
          /* 			  fprintf(stderr,"for igroup = %d nchild =
            ...
            do {
              ...
              if (thischild->Mtot / bigchild->Mtot >= MinMergerRatio) {
                ...
                */
                ...
                fprintf(Fof,
                        "%4d   %14" STR_FMT
                        "   %14.4f   %8d      %8d    %14" STR_FMT
                        "   %14" STR_FMT "    %14.4f   %14.4f     \n",
                        isnapshot, node[igroup].haloid, node[igroup].Mtot,
                        bigchild->snapshot, thischild->snapshot,
                        bigchild->haloid, thischild->haloid, bigchild->Mtot,
                        thischild->Mtot);
              }
            } while (thischild->Sibling != NULL);
          } else {
  ...
  /*Here on I just repeat the previous steps -- and produce all the mergers that
   * are not complete */
  ...
  fprintf(Fof, "###############################################################"
               "###################################################\n");
  fprintf(Fof, "# Snap        FofHaloid         FofMtot             ChId       "
               "    ChMtot       PrevFofHaloId     PrevFofHaloSnap \n");
  fprintf(Fof, "#  i              l                d                 l         "
               "       d              l                  i         \n");
  fprintf(Fof, "###############################################################"
               "###################################################\n");
  ...
  for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM;
       isnapshot <= PARAMS.MAX_SNAPSHOT_NUM; isnapshot++) {
    my_progressbar(isnapshot - PARAMS.MIN_SNAPSHOT_NUM, &interrupted);
    /*       fprintf(stderr,"\n\n Now writing out all mergers that started at
     * snapshot %d Ngroups = %"STR_FMT" \n",isnapshot,Ngroups[isnapshot]); */
    if (Ngroups[isnapshot] > 0) {
      node = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        if (node[igroup].BigChild != NULL) {
          ...
          if ((thisfofhalo->haloid != bigchildfofhalo->haloid) &&
              (thisfofhalo->haloid != node[igroup].haloid)) {
            if (node[igroup].Mtot / thisfofhalo->Mtot >= MinMergerRatio) {
              /* If I can't assign these mergers in make_fof_tree -> assign them
               * here.*/
              ...
              fprintf(Fof,
                      "%4d   %14" STR_FMT "   %14.4f   %14" STR_FMT
                      "    %14.4f    %12" STR_FMT "  %12d\n",
                      isnapshot, thisfofhalo->haloid, thisfofhalo->Mtot,
                      node[igroup].haloid, node[igroup].Mtot,
                      bigchildfofhalo->haloid, bigchildfofhalo->snapshot);
            }
          }
        }
      }
...
void cumulative_merger_history(struct node_data *tree[], int64 *Ngroups) {
  ...
  /* PRINTSTEP =
   * (int)floor(0.1*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM-1)); */
  /* SMALLPRINTSTEP =
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM-1)) > 1 ?
   * ceil(0.01*(PARAMS.MAX_SNAPSHOT_NUM-PARAMS.MIN_SNAPSHOT_NUM-1)):1; */
  init_my_progressbar(PARAMS.MAX_SNAPSHOT_NUM - PARAMS.MIN_SNAPSHOT_NUM + 1,
                      &interrupted);
  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
    ...
    my_progressbar(PARAMS.MAX_SNAPSHOT_NUM - isnapshot, &interrupted);
    if (Ngroups[isnapshot] > 0) {
      my_snprintf(outfname, MAXLEN, "%s/%s%03d%s", PARAMS.OUTPUT_DIR,
                  "cumulative_mergers_", isnapshot, ".txt");

      /* 	  fprintf(stderr,"About to  open file %s\n",outfname); */
      fp = my_fopen(outfname, "w");
      fprintf(fp, "############################################################"
                  "#############################\n");
      fprintf(fp, "#   Snapshot       HaloId         Mtot      CumulNmergers   "
                  "   CumulNflybys      FormationZ  \n");
      fprintf(fp, "#      i              l             d           l           "
                  "      l              f       \n");
      fprintf(fp, "############################################################"
                  "#############################\n");
      ...
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (thisnode->isFof == 1) {
          totnmergers = 0;
          totnflybys = 0;
          /* 			  totnmergers += (thisnode->Nmergers +
           * thisnode->NDisruptions + thisnode->NDissolutions); */
          ...
          while (thisnode != NULL) {
             ...
             */
            /* 					{ */
            /* 					  fprintf(stderr,"this should not have happened..i have
             * already visited this node\n"); */
            /* 					  fprintf(stderr,"thisnode->haloid = %"STR_FMT"
             * thisnode->snapshot = %d  groupnum = %"STR_FMT"
             ...
             */
            totnmergers += (thisnode->Nmergers + thisnode->NDisruptions +
                            thisnode->NDissolutions);
...
void output_interesting_halos(struct node_data *tree[], int64 *Ngroups) {
  ...
  my_snprintf(outfname, MAXLEN, "%s/%s", PARAMS.OUTPUT_DIR,
              "interesting_halos.txt");
  fp = my_fopen(outfname, "w");
  fprintf(fp, "#Sub-sub halos where the subhalo disappears but the sub-sub "
              "doesn't \n\n");
  fprintf(fp, "################################################################"
              "##############################################\n");
  fprintf(fp, "#   Snapshot       HaloId         Mtot      ParentLevel     "
              "ContainerHaloId     NodeFormZ      ContainerFormZ \n");
  fprintf(fp, "#      i              l            d            i               "
              "   l                f                 f       \n");
  fprintf(fp, "################################################################"
              "##############################################\n");
      ...
      for (igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (thisnode->ParentLevel > 2) {
          ...
          if (container->Parent != NULL && thisnode->Parent != NULL) {
            if (container->Parent->haloid != container->haloid &&
                thisnode->Parent->haloid == thisnode->haloid)
              fprintf(fp,
                      "%10d  %12" STR_FMT "   %12.4g    %10d    %12" STR_FMT
                      "    %12.4f   %12.4f\n",
                      isnapshot, thisnode->haloid, thisnode->Mtot,
                      thisnode->ParentLevel, container->haloid,
...
void output_suspicious_halos(struct node_data *tree[], int64 *Ngroups) {
  ...
  my_snprintf(outfname, MAXLEN, "%s/%s", PARAMS.OUTPUT_DIR,
              "suspicious_halos_mratio.txt");
  fp = my_fopen(outfname, "w");
  fprintf(fp, "######################################################\n");
  fprintf(fp, "# Mthresh = %12.4g  mass ratio = %f \n", Mthresh, Mratio);
  fprintf(fp, "################################################################"
              "#######################################\n");
  fprintf(fp, "# Snapshot       FofHaloId       FofMass          BigSubId      "
              "BigSubMass     FofFormZ    BigSubFormZ \n");
  fprintf(fp, "#    i               l              d                l          "
              "    d              f            f      \n");
  fprintf(fp, "################################################################"
              "#######################################\n");
  ...
  for (isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
    ...
    if (Ngroups[isnapshot] > 0) {
      ...
      while (ifof < Ngroups[isnapshot] && (&node[ifof]) != NULL) {
        ...
        if (thisnode->Nsub > 1) {
          bigsub = &node[ifof + 1];
          if ((thisnode->Mtot > Mthresh) &&
              bigsub->Mtot / thisnode->Mtot > Mratio)
            fprintf(fp,
                    "%10d  %12" STR_FMT "   %12.4g     %12" STR_FMT
                    "   %12.4g    %12.4f   %12.4f\n",
                    isnapshot, thisnode->haloid, thisnode->Mtot, bigsub->haloid,
                    bigsub->Mtot, thisnode->FormationRedshift,
  ...
  fprintf(fp, "################################################################"
              "######################################\n");
  fprintf(fp, "# Snapshot    GroupNum      HaloId         Mtot     "
              "NextSnapshot      NextGroupNum      MtotNextSnap \n");
      ...
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        ...
        if (thisnode->haloid >= ifof) {
          while (thisnode != NULL && thisnode->BigChild != NULL) {
            /* mass gain and the smaller of the two needs to satisfy the mass
             * threshold */
            if (thisnode->BigChild->Mtot / thisnode->Mtot < Mratio &&
                thisnode->BigChild->Mtot > Mthresh)
              fprintf(fp,
                      "%6d   %12" STR_FMT " %12" STR_FMT
                      "  %12.4g   %10d  %12" STR_FMT "  %13.4g \n",
                      thisnode->BigChild->snapshot, thisnode->BigChild->nodeloc,
                      thisnode->BigChild->haloid, thisnode->BigChild->Mtot,
                      thisnode->snapshot, thisnode->nodeloc, thisnode->Mtot);

            /* mass loss and the smaller of the two needs to satisfy the mass
             * threshold */
            if (thisnode->BigChild->Mtot / thisnode->Mtot > (1.0 / Mratio) &&
                thisnode->Mtot > Mthresh)
              fprintf(fp1,
                      "%6d   %12" STR_FMT " %12" STR_FMT
                      "  %12.4g   %10d   %12" STR_FMT " %13.4g \n",
                      thisnode->BigChild->snapshot, thisnode->BigChild->nodeloc,
  ...
  fprintf(fp1,
          ...
          "#############################################################\n");
  fprintf(fp1,
          "# Snapshot          Groupnum          Haloid          Mtot       "
          "ParentLevel       ContainerHaloId    Parenthaloid    ParentMass \n");
  fprintf(fp1,
          "#    i                 l                 l             d           "
          "i                    l                   l              d     \n");
  fprintf(fp1,
  ...
  my_snprintf(outfname, MAXLEN, "%s/%s", PARAMS.OUTPUT_DIR,
              "suspicious_halos_disappear.txt");
  ...
  for (isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
    ...
    if (Ngroups[isnapshot] > 0) {
      ...
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        thisnode = &node[igroup];
        if (thisnode->Mtot > Mthresh && thisnode->BigChild == NULL &&
            thisnode->isFof == 0) {
          fprintf(fp,
                  "%10d   %14" STR_FMT "   %14" STR_FMT
                  "   %12.6g      %14" STR_FMT "   %12.4g    %12" STR_FMT " \n",
                  thisnode->snapshot, thisnode->nodeloc, thisnode->haloid,
                  thisnode->Mtot, thisnode->FofHalo->haloid,
        ...
        if (thisnode->Parent != NULL &&
          ...
          fprintf(fp1,
                  "%10d   %14" STR_FMT "   %14" STR_FMT
                  "   %12.4g    %12hd    %14" STR_FMT "    %12" STR_FMT
                  "  %12.6g   \n",
                  thisnode->snapshot, thisnode->nodeloc, thisnode->haloid,
                  thisnode->Mtot, thisnode->ParentLevel,
                  thisnode->ContainerHalo->haloid, thisnode->Parent->haloid,
                  thisnode->Parent->Mtot);
        }
...
void print_flyby_future_header(FILE *fp) {
  ...
  fprintf(fp,
          ...
          "#############################################################\n");
  fprintf(fp,
          "#    snapshot       redshift              haloid       groupnum     "
          " mtot         boundfofmtot       subhaloid        subgroupnum       "
          "submtot       subboundfofmtot         tag          comesback \n");
  fprintf(fp,
          "#       i               f                   l             l         "
          "  d                 d                l                 l            "
          "   d                  d                 i             i      \n");
...
void output_flyby_futures(struct node_data *tree[], int64 *Ngroups) {
  ...
  my_snprintf(outfname, MAXLEN, "%s/flybys_thatcomeback.txt",
              PARAMS.OUTPUT_DIR);
  fp1 = my_fopen(outfname, "w");
  fprintf(
      fp1,
      "# Min. halo mass = %lf max. halo mass = %lf min. formation z = %f \n",
      Mmin, Mmax, MinFormZ);
  print_flyby_future_header(fp1);

  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
       isnapshot > PARAMS.MAX_SNAPSHOT_NUM - 2; isnapshot--) {
    if (Ngroups[isnapshot] > 0) {
      BaseNode = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        mwnode = &BaseNode[igroup];
        if (mwnode->isFof == 1 && mwnode->Mtot >= Mmin &&
            mwnode->Mtot <= Mmax && mwnode->FormationRedshift > MinFormZ) {
          /* All right, so we have a MW type halo at z ~ 0 */
          flag = 0;
          while (mwnode != NULL) {
            if (mwnode->isFof == 0) {
              flag = 1; /* So the halo was a subhalo at some point in the
                           past..reject */
              break;
            }
          ...
          if (flag != 1) {
            mwnode = &BaseNode[igroup];
            my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s",
                        PARAMS.OUTPUT_DIR, "MW_flybys", mwnode->haloid, ".txt");
            fp = fopen(outfname, "r"); /* so as to not overwrite halos written
                                          at the higher snapshot */
            if (fp != NULL) {
              fclose(fp);
              flag = 1;
            }
          ...
          if (flag != 1) {
            ...
            while (mwnode != NULL) {
              if (mwnode->nodeloc < (Ngroups[mwnode->snapshot] - 1))
                ...
                break; /* to protect when primary fof does not have any subs and
                          is the last group in the snapshot */

              while (thisnode != NULL && thisnode->isFof == 0 &&
                     thisnode->nodeloc < (Ngroups[thisnode->snapshot] - 1) &&
                     thisnode->FofHalo == mwnode) {
                /* 							  tag =
                 * get_tag(thisnode,mwnode,&destructionz,&destructionradius);/\*disregard
                 * the last two things*\/ */
                // MS 7th Dec, 2011 - updated to use the new tagging system that
                // accounts for subhalos missing snapshots
                ...
                tag =
                    get_tag(thisnode, mwnode, &destructionz, &destructionradius,
                            &destruction_snapshot, &destruction_subtmot,
                            &destruction_parentmtot, &destruction_mstar);
                if (tag == 3 ||
                    tag == 5) { /* flyby tags -> should make it into a function
                                   that returns the tags from a name */
                  nflybys++;
                  comesback = 0;

                  /* Now lets see all the descendants of thisnode (while
                   * conserving haloid) and see if it comes back */
                  tmpnode = thisnode;
                  while (tmpnode != NULL && tmpnode->isFof == 0)
                    tmpnode = tmpnode->Parent; /*walk to the future when
                                                  thisnode becomes a FOF*/

                  while (tmpnode != NULL &&
                         tmpnode->haloid == thisnode->haloid) {
                    if (tmpnode->isFof == 0 &&
                        tmpnode->FofHalo->haloid == mwnode->haloid) {
                      comesback++;
                      break;
                    }
                  ...
                  /* What happens if I take out the haloid requirement ->
                     thisnode dissolves in something else which then has a
                     merger with mwnode (NOTE that mwnode will be around till
                     z~0)*/
                  tmpnode = thisnode;
                  ...
                  fprintf(fp,
                          "%10d    %14.4f   %14" STR_FMT " %14" STR_FMT
                          " %16.4lf  %16.4lf %14" STR_FMT "  %14" STR_FMT
                          "  %16.4lf   %16.4lf   %12d  %12d \n",
                          mwnode->snapshot, mwnode->z, mwnode->haloid,
                          mwnode->nodeloc, mwnode->Mtot, mwnode->BoundFofMtot,
                          thisnode->haloid, thisnode->nodeloc, thisnode->Mtot,
                          thisnode->BoundFofMtot, tag, comesback);
  ...
  /*   nwritten = snprintf(command,MAXLEN,"ls -1
   * %s/MW_flybys*.txt",PARAMS.OUTPUT_DIR); */
...
void output_milkyway_halos(struct node_data *tree[], int64 *Ngroups) {
  ...
  for (int isnapshot = PARAMS.MAX_SNAPSHOT_NUM;
       isnapshot > PARAMS.MAX_SNAPSHOT_NUM - 1; isnapshot--) {
    if (Ngroups[isnapshot] > 0) {
      node = tree[isnapshot];
      for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++) {
        thisnode = &node[igroup];
        if (thisnode->ParentLevel == 1 && thisnode->Mtot >= Mmin &&
            thisnode->Mtot <= Mmax && thisnode->FormationRedshift > MinFormZ) {
          /* All right, so we have a MW type halo at z ~ 0 */
          flag = 0;
          fof_mwcount++;
          while (thisnode != NULL) {
          ...
          thisnode = &node[igroup];
          my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s",
                      PARAMS.OUTPUT_DIR, "MW", thisnode->haloid, ".txt");
          fp = fopen(outfname, "r"); /* so as to not overwrite halos written at
                                        the higher snapshot */
          if (fp != NULL) {
            fclose(fp);
            flag = 1;
          ...
          if (flag != 1) {
            fp = my_fopen(outfname, "w");
            fprintf(fp,
                    "# MW type halo. Haloid = %" STR_FMT
                    " final snapshot = %d  mtot = %g  formationz = %12.5g\n",
                    thisnode->haloid, isnapshot, thisnode->Mtot,
                    thisnode->FormationRedshift);
            fprintf(fp, "######################################################"
                        "######################################################"
                        "#################################\n");
            fprintf(fp, "# Snapshot       GroupNum            Haloid           "
                        " Mtot       ParentLevel       Fofhaloid      ParentID "
                        "     ParentSnap    CumulNmergers \n");
            fprintf(fp, "#     i              l                  l             "
                        "   d            i                 l              l    "
                        "         i               l       \n");
            fprintf(fp, "######################################################"
            ...
            while (thisnode != NULL) {
              ...
              fprintf(fp,
                      " %6d    %14" STR_FMT "    %14" STR_FMT
                      "   %14.4g    %10d     %14" STR_FMT "     %10" STR_FMT
                      "   %10d   %14u  \n",
                      thisnode->snapshot, thisnode->nodeloc, thisnode->haloid,
                      thisnode->Mtot, thisnode->ParentLevel,
                      thisnode->FofHalo->haloid, parentid, parentsnap,
                      thisnode->CumulativeNmergers);
            ...
            mwnode = &node[igroup];
            my_snprintf(outfname, MAXLEN, "%s/%s_%" STR_FMT "%s",
                        PARAMS.OUTPUT_DIR, "MW_allsubhalos", thisnode->haloid,
                        ".txt");
            fp = my_fopen(outfname, "w");
            fprintf(fp,
                    "# MW type halo. Haloid = %" STR_FMT
            ...
            fprintf(fp, "######################################################"
                        ...
                        "########################################\n");
            fprintf(fp, "# Snapshot       GroupNum            Haloid           "
                        " Mtot       ParentLevel       FormationZ     "
                        "CumulNmergers        ParentID         ParentSnap \n");
            fprintf(fp, "#     i              l                  l             "
                        "   d            i                 f               l   "
                        "                l                i      \n");
            fprintf(fp, "######################################################"
            ...
            while (mwnode != NULL) {
              ...
              for (int64 isub = mwnode->nodeloc;
                   isub < (mwnode->nodeloc + mwnode->Nsub); isub++) {
                ...
                fprintf(fp,
                        " %6d    %14" STR_FMT "    %14" STR_FMT
                        "   %14.4g    %10d   %14.4g   %14u    %14" STR_FMT
                        "     %10d \n",
                        thisnode->snapshot, thisnode->nodeloc, thisnode->haloid,
                        thisnode->Mtot, thisnode->ParentLevel,
                         ...
                         */
                        thisnode->FormationRedshift,
                        thisnode->CumulativeNmergers, parentid, parentsnap);
                if (thisnode->FofHalo != mwnode) {
                  fprintf(stderr, "This should not have happened: Inside milky "
                                  "way type halos routine \n");
                  fprintf(stderr, "The subhalo claims to have some other halo "
                                  "as the FOF ...exiting\n");
                  exit(EXIT_FAILURE);
                }
              }
  ...
  fprintf(stderr,
          "Number of FOF MW type halos found at z = 0.0 is : %d  all MW (may "
          "have been a subhalo at some point) = %d \n",
          mwcount, fof_mwcount);
}
...
void output_gill_data(struct node_data *tree[], int64 *Ngroups) {
  ...
  for (int64 igroup = 0; igroup < Ngroups[snapshot]; igroup++) {
    ...
    if (clusternode->isFof == 1 && clusternode->BoundFofMtot >= Mmin &&
        halo_is_always_fof(clusternode) == 1) {
      ...
      Nclusters++;
      snprintf(message, MAXLEN, "%s",
               "cluster_haloids in function output_gill_data");
      cluster_haloids =
          my_realloc(cluster_haloids, sizeof(int64), Nclusters, message);
      cluster_haloids[Nclusters - 1] = clusternode->haloid;

      snprintf(outfname, MAXLEN, "%s/gill_data_%" STR_FMT ".txt",
               PARAMS.OUTPUT_DIR, clusternode->haloid);
      fp = my_fopen(outfname, "w");
      ...
      for (int64 jgroup = 0; jgroup < Ngroups[snapshot]; jgroup++) {
        if (jgroup != igroup) {
          ...
          if (tmp_dmin < Max_Search_Radius) {
            ...
            if (thisnode != NULL &&
                thisnode->FofHalo->haloid == clusternode->haloid) {
              tag = -1;
              Nsubhaloids++;
              snprintf(message, MAXLEN, "%s",
                       "subhaloids in function output_gill_data");
              subhaloids =
                  my_realloc(subhaloids, sizeof(int64), Nsubhaloids, message);
              subhaloids[Nsubhaloids - 1] = thisnode->haloid;

              snprintf(message, MAXLEN, "%s",
                       "subhalos in function output_gill_data");
              subhalos = my_realloc(subhalos, sizeof(struct node_data *),
                                    Nsubhaloids, message);
              subhalos[Nsubhaloids - 1] =
                  &node[jgroup]; // store the pointer at z=0

              // now let's go back to when the subhalo first appeared inside
              // this cluster
              while (thisnode->BigChild != NULL &&
                     thisnode->BigChild->FofHalo->haloid == clusternode->haloid)
                thisnode = thisnode->BigChild;
              ...
              if (thisnode != NULL &&
                ...
                tag = get_tag(thisnode, thisnode->FofHalo, &destructionz,
                              &destructionradius, &destruction_snapshot,
                              &destruction_subtmot, &destruction_parentmtot,
                              &destruction_mstar);
              }

              snprintf(message, MAXLEN, "%s",
                       "alltags in function output_gill_data");
      ...
      // Now all the subhalos that have interacted with this cluster have been
      // identified.
      fprintf(stderr,
      ...
      fprintf(fp, "############################################################"
                  ...
                  "############################################################"
                  "######################################################\n");
      fprintf(fp, "#  ClusterId        SubhaloId         Rsep       Rvir1      "
                  "   Rvir2        MinRsep     Rvir1_at_min    Rvir2_at_min    "
                  " Snapshot_at_min    InfallMass     FinalMass     tag      "
                  "dmax   Rvir1_at_max    Rvir2_at_max   Snapshot_at_max   \n");
      fprintf(fp, "#     l                 l               f          f        "
                  "     f             f             f              f           "
                  "       i                f              f          i         "
                  "f         f             f                i\n");
      ...
      for (int64 isub = 0; isub < Nsubhaloids; isub++) {
        ...
        while (subnode != NULL &&
               subnode->FofHalo->haloid == clusternode->haloid) {
        ...
        // MS - Added 06/22/12
        // repeat for finding the max separation. but now the sub doesn't have
        // to contained inside the halo.
        subnode = subhalos[isub];
        while (subnode != NULL) {
        ...
        // so now subnode is at the last time it was a subhalo..now let's walk
        // into the future and find the maximum separation between the sub and
        // the cluster node.
        ...
        // new scope
        {
          ...
          if (tmp_fof->haloid != clusternode->haloid) {
            fprintf(stderr,
                    "ERROR: (in output_gill_data) Expected to have cluster "
                    "with haloid = %" STR_FMT
                    " instead I have haloid = %" STR_FMT "..exiting\n",
                    clusternode->haloid, tmp_fof->haloid);
            exit(EXIT_FAILURE);
          }
          // make sure that the subhalo is still itself. (It should be -- since
          // the sub survives to the last snapshot -- where the haloids are
          // assigned.)
          while (subnode != NULL && subnode->haloid == subhaloids[isub] &&
                 tmp_fof != NULL && tmp_fof->haloid == clusternode->haloid) {
        ...
        subnode = subhalos[isub];
        fprintf(fp,
                "%14" STR_FMT " %14" STR_FMT
                "  %14.6f     %14.6f  %14.6f  %14.6f   %14.6f   %14.6f  %10d   "
                "%14.6f  %14.6f %10d  %14.6f  %14.6f  %14.6f  %10d\n",
                clusternode->haloid, subnode->haloid,
                get_separation_between_centres(subnode, clusternode),
                subnode->Rvir, clusternode->Rvir, dmin, rvir1_at_min,
                rvir2_at_min, snapshot_for_dmin, subnode->InfallMass,
                subnode->Mtot, alltags[isub], dmax, rvir1_at_max, rvir2_at_max,
                snapshot_for_dmax);
