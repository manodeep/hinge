...
double get_impulse(struct node_data *g, struct node_data *f, float rsep,
  ...
  Impulse =
      (G * pow(Rvir, 3.0) * pow(Mtot2, 2.0)) /
      (3.0 * pow(rsep, 4.0) * pow(vsep, 2.0) * Conc * pow(gc, 2.0) * Mtot1);
  //  fprintf(stderr,"in Impulse %f %f %f %f %f %f %f\n",gc, Mtot1, Mtot2, Rvir,
  //  Conc, rsep, vsep);
  return Impulse;
}

double get_external_delpot_prim(struct node_data *secondary,
                                struct node_data *primary, float rsep,
                                float vsep) {
  ...
  if (secondary->haloid == primary->haloid &&
      secondary->snapshot == primary->snapshot)
    DelPotPrim = 0.0;
  else {
    ...
    if (Omega <= 0.5) {
      DelPotPrim = 1.3e-3 * (pow(Mtot2, 2.0) / pow(Mtot1, 2.0)) *
                   (pow(Rvir1, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 0.5) && (Omega <= 1.0)) {
      DelPotPrim = 7e-4 * (pow(Mtot2, 2.0) / pow(Mtot1, 2.0)) *
                   (pow(Rvir1, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 1.0) && (Omega <= 2.5)) {
      DelPotPrim = 4.7e-4 * (pow(Mtot2, 2.0) / pow(Mtot1, 2.0)) *
                   (pow(Rvir1, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 2.5) && (Omega <= 5.0)) {
      DelPotPrim = 2.8e-4 * (pow(Mtot2, 2.0) / pow(Mtot1, 2.0)) *
                   (pow(Rvir1, 6.0) / pow(rsep, 6.0));
    } else if (Omega > 5.0) {
      DelPotPrim = 0.0;
    }
...
double get_external_delpot_sec(struct node_data *secondary,
                               struct node_data *primary, float rsep,
                               float vsep) {
  ...
  if (secondary->haloid == primary->haloid &&
    ...
    if (Omega <= 0.5) {
      DelPotSec = 1.3e-3 * (pow(Mtot1, 2.0) / pow(Mtot2, 2.0)) *
                  (pow(Rvir2, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 0.5) && (Omega <= 1.0)) {
      DelPotSec = 7e-4 * (pow(Mtot1, 2.0) / pow(Mtot2, 2.0)) *
                  (pow(Rvir2, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 1.0) && (Omega <= 2.5)) {
      DelPotSec = 4.7e-4 * (pow(Mtot1, 2.0) / pow(Mtot2, 2.0)) *
                  (pow(Rvir2, 6.0) / pow(rsep, 6.0));
    } else if ((Omega > 2.5) && (Omega <= 5.0)) {
      DelPotSec = 2.8e-4 * (pow(Mtot1, 2.0) / pow(Mtot2, 2.0)) *
                  (pow(Rvir2, 6.0) / pow(rsep, 6.0));
    } else if (Omega > 5.0) {
      DelPotSec = 0.0;
    }
...
double get_internal_delpot_prim(struct node_data *secondary,
                                struct node_data *primary, float rsep,
                                float vsep) {
  ...
  if (secondary->haloid == primary->haloid &&
    ...
    if (fabs(rsep) > 0.0) {
      if (rsep >= primary->Rvir_anyl ||
          ratio > 2.0) /* quite far flung -> may be ~Rvir -> use external
                          prescription */
      {
...
double get_internal_delpot_sec(struct node_data *secondary,
                               struct node_data *primary, float rsep,
                               float vsep) {
  /* When the perturber is inside. Table 2 of Vesperini & Weinberg 1999 */
  ...
  if (secondary->haloid == primary->haloid &&
    ...
    if (fabs(rsep) > 0.0) {
      if (rsep >= secondary->Rvir_anyl ||
          ratio > 2.0) /* quit far flung -> may be ~Rvir -> use external
                          prescription */
      {
      ...
      DelPotSec = (double)
          big_perturbation; /*rsep should only be 0 only when the primary and
