...
//
//  a currently sparse set of functions designed to figure out 'physically important' flyby encounters.
//
...
double get_impulse(struct node_data *g,struct node_data *f, float rsep, float vsep)
{
  ...
  Impulse=(G*pow(Rvir,3.0)*pow(Mtot2,2.0))/(3.0*pow(rsep,4.0)*pow(vsep,2.0)*Conc*pow(gc,2.0)*Mtot1);
  //  fprintf(stderr,"in Impulse %f %f %f %f %f %f %f\n",gc, Mtot1, Mtot2, Rvir, Conc, rsep, vsep);
  return Impulse;
}
...
/*   double R_half = primary->Rvir_anyl*half_mass_factor; /\*very approx. Should use eqn. 28 in Lokas 2001. However that  */
/* 														  needs a proper c -- which has not yet been implemented*\/ */
...
/*   if (float_almost_equal(rsep,0.0,maxUlps) == 0) /\* i.e, rsep != 0.0 *\/ */
  ...
  if(secondary->haloid==primary->haloid && secondary->snapshot == primary->snapshot)
  ...
  else
	{
	  if(fabs(rsep) > 0.0)
		{
		  if(rsep >= primary->Rvir_anyl || ratio > 2.0) /* quite far flung -> may be ~Rvir -> use external prescription */
			{
...
/*   const float half_mass_factor = 0.5; */
/*   double R_half = secondary->Rvir_anyl*half_mass_factor; /\*very approx. Should use eqn. 28 in Lokas 2001. However that  */
/* 														  needs a proper c -- which has not yet been implemented*\/ */
...
/*   if (float_almost_equal(rsep,0.0,maxUlps) == 0) /\* i.e, rsep != 0.0 *\/ */
  ...
  if(secondary->haloid==primary->haloid && secondary->snapshot == primary->snapshot)
  ...
  else
	{
	  if(fabs(rsep) > 0.0)
		{
		  if(rsep >= secondary->Rvir_anyl || ratio > 2.0) /* quit far flung -> may be ~Rvir -> use external prescription */
			{
		...
		DelPotSec = (double)big_perturbation; /*rsep should only be 0 only when the primary and secondary are the same, i.e., dissolving in case of a merger or a detachment in a flyby */
	}
