...
void output_plot_data(struct node_data *tree[], int64 *Ngroups)
{
    ...
    int bins_noprev_fof[Nbins1], bins_noprev_sub[Nbins1], bins_nonext_fof[Nbins1], bins_nonext_sub[Nbins1],
        bins_none_fof[Nbins1], bins_none_sub[Nbins1];
    ...
    for (int isnapshot = PARAMS.MIN_SNAPSHOT_NUM; isnapshot <= PARAMS.MAX_SNAPSHOT_NUM; isnapshot++)
    {
        ...
        if (Ngroups[isnapshot] > 0)
        {
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup++)
            {
                ...
                if (thisnode->VisitedForMassloss == 0 && (thisnode->Mtot / Mthresh) > Mratio &&
                    (thisnode->Mtot / Mthresh) < (1.0 + Mratio))
                {
                    ...
                    while (thisnode != NULL && thisnode->haloid == haloid)
                    {
                        ...
                        fprintf(fp,
                                "%6d     %14" STR_FMT "   %14" STR_FMT "   %12.4g   %10hd    %14" STR_FMT
                                "  %12" STR_FMT "    %14" STR_FMT "  %12.4g    %14.4g  \n",
                                thisnode->snapshot, thisnode->nodeloc, thisnode->haloid, thisnode->Mtot,
                                thisnode->ParentLevel, thisnode->ContainerId, thisnode->Nsub, thisnode->FofHalo->haloid,
                                fofmtot, separation);
            ...
            for (int i = 0; i < Nbins1; i++)
                fprintf(fp3,
                        ...
                        isnapshot, i * binsize + log10_Mmin, bins_noprev_fof[i], bins_noprev_sub[i], bins_nonext_fof[i],
                        bins_nonext_sub[i], bins_none_fof[i], bins_none_sub[i], bins_ngroups_fof[i],
                        bins_ngroups_sub[i]);
        }
        fprintf(fp1, "%10d    %14" STR_FMT " \n", isnapshot, Nmissing);
        fprintf(fp2, "%10d  %12" STR_FMT "    %12" STR_FMT "   %12" STR_FMT "   %12" STR_FMT " \n", isnapshot, noprev,
                nonext, none, Ngroups[isnapshot]);
    }
                    ...
                    for (int64 second = first + 1; second < fofnum + fofnode->Nsub; second++)
                    {
                        ...
                        if (secondnode->Mtot / firstnode->Mtot >= MinMassRatio)
                        {
                            if (check_for_flyby(firstnode, secondnode, &rsep, &vsep) == 1)
                            {
                                ...
                                if (secondnode->BigChild != NULL && secondnode->BigChild->ParentLevel >= 2 &&
                                    secondnode->BigChild->ParentLevel < secondnode->ParentLevel &&
                                    secondnode->Parent != NULL && secondnode->Parent->haloid == secondnode->haloid &&
                                    secondnode->Parent->ParentLevel < secondnode->ParentLevel && firstnode->isFof == 0)
                                    tag = 3;
                                else
                                {
                                ...
                                */
                                ...
                                fprintf(fp,
                                        ...
                                        "  %14.6g    %14.6g     %12.4f    %12.4f   %14" STR_FMT "   %14" STR_FMT
                                        "   %10.4g    %10.4g   %10.4g  %12d\n",
                                        firstnode->snapshot, firstnode->nodeloc, secondnode->nodeloc, firstnode->Mtot,
                                        secondnode->Mtot, rsep, vsep, firstnode->ContainerId, secondnode->ContainerId,
                                        Impulse, DelPotPrim, DelPotSec, tag);
                            }
                        }
                    }
                }
    ...
    */
    ...
    for (int i = 0; i < NOUTPUTS; i++)
    {
        if (Snapshots[i] <= PARAMS.MAX_SNAPSHOT_NUM && Snapshots[i] >= PARAMS.MIN_SNAPSHOT_NUM)
        {
            ...
            for (int j = 0; j <= Nbins2[i]; j++)
            {
                ...
                {
                    fprintf(fp, "%8d   %10d   %10" STR_FMT " %14.4g  %14.4g   %14.4g  \n", Snapshots[i], j, nhalos[j],
                            log_mass_limits[0] + log_mass_bin * j, halfz[j] / (double)nhalos[j],
                            sqrt(halfz_sigma[j] / (double)nhalos[j] -
                                 halfz[j] / (double)nhalos[j] * halfz[j] / (double)nhalos[j]));
                }
            }
            ...
            for (int64 igroup = 0; igroup < Ngroups[isnapshot]; igroup += fofnode->Nsub)
            {
                ...
                while ((jgroup - igroup) < fofnode->Nsub && jgroup < Ngroups[isnapshot])
                {
                    ...
                    if (thisnode->BigChild != NULL && thisnode->BigChild->isFof == 1 &&
                        thisnode->BigChild->haloid != fofnode->haloid && fofnode->BigChild != NULL &&
                        fofnode->BigChild->isFof == 1 && fofnode->BigChild->snapshot == thisnode->BigChild->snapshot &&
                        fofnode->BigChild->snapshot == (fofnode->snapshot - 1) &&
                        thisnode->FofHalo == fofnode) /*the last check is to protect against my stupidity*/
                    {
                        /* 					  tag =
                        ...
                        {
                            fprintf(fp,
                                    " %6d   %14.4e  %14" STR_FMT "  %14" STR_FMT "  %14" STR_FMT "  %14" STR_FMT
                                    "   %14.4lf    %14.4lf   %14.4lf\n",
                                    thisnode->BigChild->snapshot, REDSHIFT[thisnode->BigChild->snapshot],
                                    fofnode->BigChild->nodeloc, thisnode->BigChild->nodeloc, fofnode->BigChild->haloid,
                                    thisnode->BigChild->haloid, fofnode->BigChild->BoundFofMtot,
    ...
    int64 subhaloid;
    const double Time_Window_Factor = 1.0; // Look for interactions in the window: [t_now,
    ...
    for (int itarget = 0; itarget < N_target_redshifts; itarget++)
    {
            ...
            target_snapshots[itarget] <= PARAMS.MAX_SNAPSHOT_NUM)
        {
            ...
            for (earliest_snapshot = PARAMS.MIN_SNAPSHOT_NUM; earliest_snapshot <= PARAMS.MAX_SNAPSHOT_NUM;
                 earliest_snapshot++)
            {
            ...
            fprintf(stderr, "Age[%d] = %e Time_window=%lf Earliest_Age=%lf Age[%d] = %lf \n", last_snapshot,
                    PARAMS.Age[last_snapshot], Time_Window, Earliest_Age, earliest_snapshot,
                    PARAMS.Age[earliest_snapshot]);

            // Find the earliest snapshot that has an age lower than Earliest_age
            for (int imass = 0; imass < N_target_mass; imass++)
            {
                // Mass-bins
                my_snprintf(outfname, MAXLEN, "%s/halo_distribution_near_bighalos_imass_%02d_%03d.txt",
                            PARAMS.OUTPUT_DIR, imass, last_snapshot);
                fp = my_fopen(outfname, "w");
                fprintf(fp, "# Min mass = %lf max. mass = %lf target redshift = %f \n", Min_Target_Halo_Mass[imass],
                        Max_Target_Halo_Mass[imass], REDSHIFT[last_snapshot]);
                fprintf(fp, "##################################################################"
                            "##################################################################"
                            "##################################################################"
                            "##################################################################"
                            "###########################################\n");
                fprintf(fp, "# Snapshot        FofHaloId     ContainerHaloId       SubhaloId   "
                ...
                {
                    thisnode = &BaseNode[igroup];
                    if (thisnode->isFof == 1 && thisnode->BoundFofMtot >= Min_Target_Halo_Mass[imass] &&
                        ...
                        while (thisnode != NULL && thisnode->snapshot <= last_snapshot)
                        {
                            if (thisnode->isFof == 1)
                            {
                                ...
                                for (jgroup = 1; jgroup < thisnode->Nsub; jgroup++)
                                {
                                    ...
                                    tag = get_tag(subnode, subnode->FofHalo, &destructionz, &destructionradius,
                                                  &destruction_snapshot, &destruction_subtmot, &destruction_parentmtot,
                                                  &destruction_mstar);
                                           ...
                                           subnode->snapshot <= last_snapshot)
                                    {
                                        ...
                                        fprintf(fp,
                                                ...
                                                subnode->snapshot, tmp_fofnode->haloid, subnode->ContainerHalo->haloid,
                                                subhaloid, tmp_fofnode->Mtot, tmp_fofnode->BoundFofMtot,
                                                subnode->ContainerHalo->Mtot, subnode->Mtot, dist, vel, tag,
                                                destructionz, tmp_fofnode->Rhalf, tmp_fofnode->Rvir,
                                                tmp_fofnode->Rvir_anyl, rp, vproj, subnode->InfallMass,
                                                subnode->InfallSnapshot, subnode->Mstar, tmp_fofnode->Mstar);
...
int find_fof_flyby_future(struct node_data *const n1, struct node_data *const n2)
{
    ...
    */
    ...
    {
        if (node1->ContainerHalo == node2 || node2->ContainerHalo == node1 || node1->FofHalo == node2 ||
            node2->FofHalo == node1)
            return 0;
...
void print_header_for_subsub_flyby(FILE *fp)
{
    ...
    fprintf(fp, "####################################################################"
                "####################################################################"
                "#######################################################\n");
    fprintf(fp, "# snapshot         groupnum1          groupnum2           m1        "
