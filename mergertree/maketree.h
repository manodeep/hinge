...
struct  node_data
{
  ...
  float meanvel[3];/*all velocities are assumed to be "Gadget" snapshot velocities 
					and converted to peculiar velocities by multiplying with sqrt(a) */
  ...
  double Rvir_anyl ; /* analytically obtained Rvir for some overdensity given Mtot and z (assuming spherical halo) */
  double Rvir;/* Numerically obtained Rvir by computing the density profile and locating the overdensity radius. */
  ...
  int64 ContainerId; /* Group number that contains this halo. For a Fof, this will be the Fof itself */

#ifndef FOF_ONLY
  int64 Nsub; /* actual number of subhalos. Nsub equals to subhalos for the Fof, and the number of (sub)_n-subs for subhalos */
#endif
  int64 Nchild; /* actual number of pointers that consider this halo to be a parent */

  short VisitedForMassloss;
  short VisitedForCumulativeMerger;

  unsigned int CumulativeNmergers;/* Not sure why I have unsigned int ??!! int64 may be?*/
  ...
  struct node_data *BigChild; /* Progenitor at a previous snapshot. Similar to Parent (mostly the immediately previous snapshot but not necessarily so ) */
...
/*   struct node_data *Neighbours; */
  struct node_data * (*Neighbours)[]; //So this is a  pointer to array of pointers to struct node_data (as explained by cdecl)
};
