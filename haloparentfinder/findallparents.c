...
//private functions 
void print_fofassign(int64 thisnum,struct group_data *prevgroup,struct group_data *nextgroup,int64 NextNsub,double *NextAllRanks,int64 *NextAllCommon,const char *outpath);
void print_reassign (int64 thisnum,struct group_data *prevgroup,struct group_data *nextgroup,int64 NextNsub,double *NextAllRanks,int64 *NextAllCommon,const char *outpath);
...
void print_reassign(int64 thisnum,struct group_data *prevgroup,struct group_data *nextgroup,int64 NextNsub,double *NextAllRanks,int64 *NextAllCommon,const char *outpath)
{
  ...
  for(int64 i=0;i<NextNsub;i++) {
    if(NextAllRanks[i] > 0.0)
      fprintf(fp,"%7"STR_FMT     "%15"STR_FMT      "%12"STR_FMT     "%11d      %10"STR_FMT    "%11"STR_FMT "      %10"STR_FMT     "%14.4f   %12"STR_FMT  " %14"STR_FMT   "    %16"STR_FMT   
	      "     %12"STR_FMT   "%14"STR_FMT " \n",thisnum,prevgroup[thisnum].N,prevgroup[thisnum].Nsub,nextgroup[i].snapshot,i,nextgroup[i].N,nextgroup[i].Nsub,
	      NextAllRanks[i],NextAllCommon[i],nextgroup[i].ParentId,nextgroup[i].FOFHalo,nextgroup[nextgroup[i].FOFHalo].N,nextgroup[nextgroup[i].FOFHalo].ParentId);
...
{
  fprintf(fp,"#################################################################################################################################################################################################\n");
  fprintf(fp,"# PrevGrpNum      PrevN        PrevNsub   NextSnap     NextNum       NextN       NextNsub      NextRank        NextCommon       NextParentId      NextFOFHalo     NextFOFNp       NextFOFParent #\n");
  fprintf(fp,"#################################################################################################################################################################################################\n");
}
...
void print_fofassign(int64 thisnum,struct group_data *prevgroup,struct group_data *nextgroup,int64 NextNsub,double *NextAllRanks,int64 *NextAllCommon,const char *outpath)
{
  ...
  for(int64 i=0;i<NextNsub;i++) {
    if(nextgroup[i].isFof==1 && NextAllRanks[i] > 0.0)
      fprintf(fp,"%7"STR_FMT "      %10"STR_FMT "     %7"STR_FMT "    %6d       %10"STR_FMT "   %10"STR_FMT "    %7"STR_FMT "    %12.4f "  
	      "  %12"STR_FMT "  %14"STR_FMT "  %16"STR_FMT "  %12"STR_FMT "  %14"STR_FMT " \n",thisnum,prevgroup[thisnum].N,
	      prevgroup[thisnum].Nsub,nextgroup[i].snapshot,i,nextgroup[i].N,nextgroup[i].Nsub,
	      NextAllRanks[i],NextAllCommon[i],nextgroup[i].ParentId,nextgroup[i].FOFHalo,nextgroup[nextgroup[i].FOFHalo].N,nextgroup[nextgroup[i].FOFHalo].ParentId);
...
int64 findfofparents(struct group_data *prevgroup,int64 PrevNsub, struct group_data *nextgroup, int64 NextNsub,const char *outpath)
{
  int64 i,j;
  ...
  for(i=0;i<NextNsub;i++) {
    ...
    for(j=0;j<nextgroup[i].N;j++) {
      ...
      if(NextAllPartIds[nextgroup[i].id[j]] == 1) {
				...
				/* fprintf(stderr,"duplicate id = %"STR_ID" in group = %"STR_FMT" previously seen in group =%"STR_FMT" \n",nextgroup[i].id[j],i,NextAllRealGroupIds[nextgroup[i].id[j]]); */
				flag++;
      }
      NextAllPartIds[nextgroup[i].id[j]] = 1;
      NextAllGroupIds[nextgroup[i].id[j]] = FOF_Parent; /* Note that all the subhalos are faked as if they are located in the FOF container*/
  ...
  while(i<PrevNsub) {
    ...
    if(prevgroup[FOF_Parent].ParentId < 0) {
      j=i;
      /* 		  fprintf(stderr,"Now starting on halo with i= %d, NextNsub = %d  with FOF parent = %d  prev.fofhalo = %d \n",i,PrevNsub,FOF_Parent,prevgroup[j].FOFHalo); */
	...
	for(int64 k=0;k<prevgroup[j].N;k++) {
	  ...
	  if(tmp_id < NextMaxPartId) {
		...
		exit(EXIT_FAILURE);
	      ...
	      prevgroup[j].parentgroupforparticle[k] = NextAllRealGroupIds[tmp_id]; /* stores the real halo number and not the Fof halo number. Group matching
										       is still done based on the Fof halo number.
										    */
	      prevgroup[j].parentsnapshotforparticle[k] = nextgroup[tmp_grpid].snapshot;
	      nextgroup[NextAllRealGroupIds[tmp_id]].parentgroupforparticle[NextAllRealGroupLocs[tmp_id]] = j;
	      nextgroup[NextAllRealGroupIds[tmp_id]].parentsnapshotforparticle[NextAllRealGroupLocs[tmp_id]] = prevgroup[j].snapshot;
	    }
	  }
	}
	...
	j++;
      ...
      if(max_rankid != -1) {
	/* 
		 Only assign if this next FOF halo doesnt already have a parent. This
		 does require the groups to be processed in decreasing particle number
		 but that does not NECESSARILY hold true. 
	...
	if (nextgroup[max_rankid].ParentId < 0) {
	  ...
	  nextgroup[max_rankid].Rank = NextAllRanks[max_rankid]; /* Assumes that the rank is something that is reversible e.g., binding energy rank would not work */
	  nextgroup[max_rankid].NParents++; /* does the next halo have multiple progenitors -> initialisation in loadgroups.c is important */
	  print_fofassign(FOF_Parent,prevgroup,nextgroup,NextNsub,NextAllRanks,NextAllCommon,outpath);
	  NFofHalofound++;
	  
	} else {
	  /* 
	     some other FOF halo already claims that max_rankid is the parent halo for them. Now
	     I have to compare the goodness of the match and determine the true child.
	  ...
	  */
	  if(nextgroup[max_rankid].Rank < NextAllRanks[max_rankid]) {
	    ...
	    print_fofassign(FOF_Parent,prevgroup,nextgroup,NextNsub,NextAllRanks,NextAllCommon,outpath);
	    /* 					  NFofHalofound++; */ /* Dont update NFofHalofound since one halo is being rejected */
	    
	  } else {
	    /* 
	       Here I could potentially assign it to some other FOF group. But I will
	       leave this prevgroup[i] halo to be assigned by subfind.
	    */
...
int64 findallparents(struct group_data* prevgroup,int64 PrevNsub,struct group_data* nextgroup,int64 NextNsub,const int snapshot,const char *outpath)
{
  ...
  */
  ...
  for(int64 i=0;i<PrevNsub;i++) {
	...
	tmp_grpid = -1;
	if(tmp_id < NextMaxPartId)  { /* NextMaxPartId is actually 1 greater than the actual max particle id. Hence the < instead of an <= */
	  if(NextAllPartIds[tmp_id] == 1) {
	    tmp_grpid = NextAllGroupIds[tmp_id];
	    NextAllCommon[tmp_grpid]++;
	    
	    if((PARAMS.MAX_RANK_LOC <= 0) || (PARAMS.MAX_RANK_LOC > 0 && j < PARAMS.MAX_RANK_LOC))//0 based indexing -> index MAX_RANK_LOC-1 is the MAX_RANK_LOC'th element
	      NextAllRanks[tmp_grpid] += compute_rank(j);

	    if((PARAMS.MAX_RANK_LOC <= 0) || (PARAMS.MAX_RANK_LOC > 0 && NextAllGroupLocs[tmp_id] < PARAMS.MAX_RANK_LOC))
	...
	/* Going to exploit the fact that nextgroup FOF finding would have resulted in their ParentId being filled up */
	
	/* 21st April, 2010: Do I need the following section ? */
	if( (prevgroup[i].isFof==1) && (prevgroup[i].Nsub > 1) && (nextgroup[max_rankid].isFof != 1) && (nextgroup[(nextgroup[max_rankid].FOFHalo)].ParentId <0)
	    && (snapshot-prevgroup[i].snapshot) == 1) {
	  print_reassign(i,prevgroup,nextgroup,NextNsub,NextAllRanks,NextAllCommon,outpath);
	  /* 				  tmp_nfound = find_nonzero_ranks(NextAllRanks,NextNsub,tmp_max_rankids); */
	  NextAllRanks[max_rankid] = 0.0;
	  tmp_max_rankid = find_max_rank(NextAllRanks,NextNsub);
	  if ( tmp_max_rankid == -1) {
	    fprintf(stderr,"\n\n WARNING: IT MAY BE THAT A FOF HALO NEEDS TO BE ASSIGNED TO A SUBHALO AFTER ALL\n\n");
	    fprintf(stderr,"original  group loc = %"STR_FMT "  the to be parent subhalo is %"STR_FMT " at snapshot %d\n ",i,max_rankid,snapshot);
	    NumNotFound++;
	  } else {
	    ...
	    fprintf(stderr,"instead prevgroup will get assigned to fof halonum = %"STR_FMT" with rank = %g\n",nextgroup[max_rankid].FOFHalo,NextAllRanks[max_rankid]);
	...
	prevgroup[i].NpartinParent = nextgroup[max_rankid].N;
	prevgroup[i].NParents = 1; /* not reqd. but for completeness sake. (don't make the code time reversible) */
