...
// private functions
void print_fofassign(int64 thisnum, struct group_data *prevgroup,
                     struct group_data *nextgroup, int64 NextNsub,
                     double *NextAllRanks, int64 *NextAllCommon,
                     const char *outpath);
void print_reassign(int64 thisnum, struct group_data *prevgroup,
                    struct group_data *nextgroup, int64 NextNsub,
                    double *NextAllRanks, int64 *NextAllCommon,
                    const char *outpath);
...
void print_reassign(int64 thisnum, struct group_data *prevgroup,
  ...
  for (int64 i = 0; i < NextNsub; i++) {
    if (NextAllRanks[i] > 0.0)
      fprintf(fp,
              "%7" STR_FMT "%15" STR_FMT "%12" STR_FMT "%11d      %10" STR_FMT
              "%11" STR_FMT "      %10" STR_FMT "%14.4f   %12" STR_FMT
              " %14" STR_FMT "    %16" STR_FMT "     %12" STR_FMT "%14" STR_FMT
              " \n",
              thisnum, prevgroup[thisnum].N, prevgroup[thisnum].Nsub,
              nextgroup[i].snapshot, i, nextgroup[i].N, nextgroup[i].Nsub,
              NextAllRanks[i], NextAllCommon[i], nextgroup[i].ParentId,
              nextgroup[i].FOFHalo, nextgroup[nextgroup[i].FOFHalo].N,
              nextgroup[nextgroup[i].FOFHalo].ParentId);
  }
...
void print_reassign_header(FILE *fp) {
  ...
  fprintf(
      fp,
      "# PrevGrpNum      PrevN        PrevNsub   NextSnap     NextNum       "
...
void print_fofassign(int64 thisnum, struct group_data *prevgroup,
  ...
  for (int64 i = 0; i < NextNsub; i++) {
    if (nextgroup[i].isFof == 1 && NextAllRanks[i] > 0.0)
      fprintf(fp,
              "%7" STR_FMT "      %10" STR_FMT "     %7" STR_FMT
              "    %6d       %10" STR_FMT "   %10" STR_FMT "    %7" STR_FMT
              "    %12.4f "
              "  %12" STR_FMT "  %14" STR_FMT "  %16" STR_FMT "  %12" STR_FMT
              "  %14" STR_FMT " \n",
              thisnum, prevgroup[thisnum].N, prevgroup[thisnum].Nsub,
              nextgroup[i].snapshot, i, nextgroup[i].N, nextgroup[i].Nsub,
              NextAllRanks[i], NextAllCommon[i], nextgroup[i].ParentId,
...
int64 findfofparents(struct group_data *prevgroup, int64 PrevNsub,
  ...
  my_snprintf(buf, MAXLEN, "rm -f %s/fofassigned_%03d.txt", outpath,
              prevgroup->snapshot);
  system(buf);
  ...
  NextMaxPartId++; /* should be able to index with NextMaxPartId -> n+1 elements
                    */
  NextAllPartIds =
      my_calloc(sizeof(*NextAllPartIds),
                NextMaxPartId); /* Note use of calloc instead of malloc */
  ...
  NextAllRealGroupLocs =
      my_malloc(sizeof(*NextAllRealGroupLocs), NextMaxPartId);
  ...
  for (i = 0; i < NextNsub; i++) {
    ...
    for (j = 0; j < nextgroup[i].N; j++) {
      ...
      NextAllGroupIds[nextgroup[i].id[j]] =
          FOF_Parent; /* Note that all the subhalos are faked as if they are
  ...
  if (flag > 0) {
    fprintf(stderr,
            "Found %" STR_FMT
            " duplicate particles. Code might not work properly\n",
            flag);
  }
    ...
    if (prevgroup[FOF_Parent].ParentId < 0) {
            ...
            if (NextAllPartIds[tmp_id] == 1) {
              ...
              prevgroup[j].parentgroupforparticle[k] =
                  NextAllRealGroupIds[tmp_id]; /* stores the real halo number
                                               ...
                                               */
              prevgroup[j].parentsnapshotforparticle[k] =
                  nextgroup[tmp_grpid].snapshot;
              nextgroup[NextAllRealGroupIds[tmp_id]]
                  .parentgroupforparticle[NextAllRealGroupLocs[tmp_id]] = j;
              nextgroup[NextAllRealGroupIds[tmp_id]]
                  .parentsnapshotforparticle[NextAllRealGroupLocs[tmp_id]] =
                  prevgroup[j].snapshot;
            }
          }
        }
      ...
      if (max_rankid != -1) {
        ...
        if (nextgroup[max_rankid].ParentId < 0) {
          ...
          nextgroup[max_rankid].Rank =
              NextAllRanks[max_rankid]; /* Assumes that the rank is something
                                           that is reversible e.g., binding
                                           energy rank would not work */
          nextgroup[max_rankid]
              .NParents++; /* does the next halo have multiple progenitors ->
                              initialisation in loadgroups.c is important */
          print_fofassign(FOF_Parent, prevgroup, nextgroup, NextNsub,
                          NextAllRanks, NextAllCommon, outpath);
          NFofHalofound++;
          ...
          */
          if (nextgroup[max_rankid].Rank < NextAllRanks[max_rankid]) {
            ...
            nextgroup[max_rankid].NpartinParent = prevgroup[i].N;
            nextgroup[max_rankid].Rank =
                NextAllRanks[max_rankid]; /* Assumes that the rank is Ncommon */
  ...
  for (int64 i = 0; i < PrevNsub; i++) {
    ...
    if (prevgroup[i].ParentId < 0) {
      ...
      for (int64 j = 0; j < prevgroup[i].N; j++) {
        ...
        tmp_grpid = -1;
        if (tmp_id < NextMaxPartId) { /* NextMaxPartId is actually 1 greater
          ...
          if (NextAllPartIds[tmp_id] == 1) {
            ...
            if ((PARAMS.MAX_RANK_LOC <= 0) ||
                (PARAMS.MAX_RANK_LOC > 0 &&
                 j < PARAMS.MAX_RANK_LOC)) // 0 based indexing -> index
            ...
            if ((PARAMS.MAX_RANK_LOC <= 0) ||
                (PARAMS.MAX_RANK_LOC > 0 &&
                 NextAllGroupLocs[tmp_id] < PARAMS.MAX_RANK_LOC))
            ...
            nextgroup[tmp_grpid]
                .parentgroupforparticle[NextAllGroupLocs[tmp_id]] = i;
            nextgroup[tmp_grpid]
                .parentsnapshotforparticle[NextAllGroupLocs[tmp_id]] =
      ...
      */
      if (max_rankid != -1) {
        ...
        if ((prevgroup[i].isFof == 1) && (prevgroup[i].Nsub > 1) &&
            (nextgroup[max_rankid].isFof != 1) &&
            (nextgroup[(nextgroup[max_rankid].FOFHalo)].ParentId < 0) &&
            (snapshot - prevgroup[i].snapshot) == 1) {
          print_reassign(i, prevgroup, nextgroup, NextNsub, NextAllRanks,
                         NextAllCommon, outpath);
          /* 				  tmp_nfound =
          ...
          if (tmp_max_rankid == -1) {
            ...
            fprintf(stderr,
                    "original  group loc = %" STR_FMT
                    "  the to be parent subhalo is %" STR_FMT
                    " at snapshot %d\n ",
                    i, max_rankid, snapshot);
            NumNotFound++;
          } else {
            fprintf(stderr,
                    "Force switching subhalo to point to fof at next step:\n");
            fprintf(stderr,
                    "prevgroupnum = %" STR_FMT "  with nextgroupnum = %" STR_FMT
                    "   \n",
                    i, max_rankid);
            fprintf(
                stderr,
                "instead prevgroup will get assigned to fof halonum = %" STR_FMT
                " with rank = %g\n",
