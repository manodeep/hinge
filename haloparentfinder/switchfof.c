...
// private function
void switch_parents(struct group_data *prev, struct group_data *next,
                    const int64 ncommon, const double rank, const int snapshot,
                    FILE *fp);
...
void switch_parents(struct group_data *prev, struct group_data *next,
                    ...
                    FILE *fp) {
  fprintf(fp,
          "\n\n switching prevgroupnum = %" STR_FMT
          " to point to nextgroupnum = %" STR_FMT " with ncommon  = %" STR_FMT
          " \n\n",
...
void check_fof_matches(struct group_data *prevgroup, int64 PrevNsub,
  ...
  for (int64 igroup = 0; igroup < NextNsub; igroup++) {
    ...
    if (nextgroup[igroup].N > PARAMS.MIN_NUMPART_IN_SWITCHFOF_HALO &&
        parentid >= 0 && parentid < PrevNsub) {
      ...
      */
      if ((double)prevgroup[parentid].N / (double)nextgroup[igroup].N <
          PARAMS.MIN_FCOMMON_SWITCHFOF_THRESH) {
        fprintf(stderr,
                "Conflict: for nextgroup = %" STR_FMT
                " nextgroup.nparents = %" STR_FMT " parentid = %" STR_FMT
                "  prevncommon = %" STR_FMT " next.N = %" STR_FMT "  \n",
                igroup, nextgroup[igroup].NParents, parentid,
                prevgroup[parentid].Ncommon, nextgroup[igroup].N);

        for (int64 i = 0; i < nextgroup[igroup].N; i++) {
          if (nextgroup[igroup].parentgroupforparticle[i] >= 0 &&
              nextgroup[igroup].parentsnapshotforparticle[i] ==
                  prevgroup->snapshot) {
        ...
        for (int64 i = 0; i < PrevNsub; i += prevgroup[i].Nsub) {
          ...
          if ((double)FeasibleNcommon[i] / (double)nextgroup[igroup].N >
              PARAMS.MIN_FCOMMON_SWITCHFOF_THRESH) {
            ...
            if (prevparentid >= 0) {
              for (int64 j = 0; j < PrevNsub; j++) {
                if ((double)prevgroup[j].Ncommon /
                            (double)nextgroup[prevparentid].N >
                        PARAMS.MIN_FCOMMON_SWITCHFOF_THRESH &&
                    prevgroup[j].ParentId == prevparentid &&
                    prevgroup[j].ParentSnapshot ==
                        nextgroup[prevparentid].snapshot &&
                    j != i) {
                  fprintf(stderr,
                          "prevgroupnum = %" STR_FMT
                          " is a valid progenitor for nextgroup=%" STR_FMT
                          " with ncommon1=%" STR_FMT
                          " so that prevfof=%" STR_FMT
                  ...
                  switch_parents(&prevgroup[i], &nextgroup[igroup],
                                 FeasibleNcommon[i], (double)FeasibleNcommon[i],
                                 snapshot, fp);
                  nswitched++;
                  flag = 1;
                  break;
  ...
  if (nswitched > 0) {
    fprintf(stderr,
            "Inside check_fof_matches:   nswitched halos = %" STR_FMT " \n",
            nswitched);
  }
}
