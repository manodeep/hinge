...
static inline void estimate_eta(const time_t t0, const float percent, int *hr,
                                int *min, int *sec) {
...
void find_hierarchy_in_fof(struct group_data *group, const int64 StartFofId) {
  ...
  if (group != NULL && group[StartFofId].isFof == 1 &&
      group[StartFofId].Nsub > 2) {
    ...
    if (Nsub > Min_Nsub_forPBar) {
      ...
      fprintf(stderr, "\n\n");
      fprintf(stderr,
              "\n Working on Fofhalo id %" STR_FMT " with %" STR_FMT
              " subhalos \n\n",
              StartFofId, Nsub);
      fprintf(stderr, " %3d%%", 0);
      fprintf(stderr, "\b|");
    ...
    t_start = time(NULL);
    for (int64 igroup = (StartFofId + Nsub - 1); igroup > StartFofId + 1;
         igroup--) {
      N_per_wedge = (int)sqrt((double)group[igroup].N) / NWEDGES;
      N_per_wedge =
          N_per_wedge > 100 ? 100 : (N_per_wedge < 2 ? 2 : N_per_wedge);
    ...
    while (flag == 1 && iter < MAXITER) {
      for (int64 igroup = StartFofId + 2; igroup < (StartFofId + Nsub);
           igroup++) {
        /* 			  fprintf(stderr,"igroup = %"STR_FMT"  startfofid =
         * %"STR_FMT"  Nsub = %"STR_FMT" \n",igroup,StartFofId,Nsub); */
        index = smallest_match[igroup - StartFofId];
        if (index != -1) {
          if (group[index].ParentLevel !=
              -1) { /* Check if the container has ParentLevel already set (this
    ...
    if (group != NULL)
      fprintf(stderr,
              "StartFofId = %" STR_FMT " and Nsub = %" STR_FMT
              " isfof = %3d ..exiting\n",
...
void find_hierarchy_level(struct group_data *group, const int64 Ngroups,
                          const char *outpath) {
  int64 FofId = 0;
  FILE *fp = NULL;
  char fname[MAXLEN];
  ...
  if (Ngroups > 0) {
    my_snprintf(fname, MAXLEN, "%s/subhalolevel_%03d.txt", outpath,
                group->snapshot);
    fp = fopen(fname, "rt");
    if (fp == NULL) {
      fp = my_fopen(fname, "w");
      ...
      while (FofId < Ngroups) {
        /*if the Fof has exactly one Nsub, i.e., the Fof halo itself, then
           ...
           loadgroups.c) */
        assert(group[FofId].Nsub >= 1 &&
               "FOF halos must have Nsub >=1 (since they contain themselves)");
        ...
        for (i = FofId; i < (FofId + group[FofId].Nsub); i++)
          fprintf(fp,
                  "%10" STR_FMT "     %10" STR_FMT "   %6hd    %10" STR_FMT
                  "    %10" STR_FMT "   %5d \n",
                  FofId, i, group[i].ParentLevel, group[i].ContainerIndex,
                  group[i].Nsub, group[i].N_per_wedge);
        ...
        if (fgets(str_line, MAXLINESIZE, fp) != NULL) {
          if (str_line[0] != comment) {
            ...
            int nread = sscanf(
                ...
                &dummy, &group[i].ParentLevel, &group[i].ContainerIndex,
                &group[i].Nsub,
            ...
            if (dummy != i) {
              fprintf(stderr,
                      "Error: While reading in file `%s' for parentlevel \n ",
                      fname);
              fprintf(stderr,
