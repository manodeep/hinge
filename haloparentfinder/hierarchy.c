...
void find_hierarchy_in_fof(struct group_data *group,const int64 StartFofId)
{
  ...
  int64 Nsub ;
  int flag = 1,iter = 0;
  const int MAXITER = 2; /* 1 iteration should be enough since small things go inside bigger things..but just in case*/
  int64 index;
  const int NWEDGES = 8; /*octants. If you want to increase it, make sure that the logic in find_octant works for the new value. */
  ...
  if(group != NULL && group[StartFofId].isFof==1 && group[StartFofId].Nsub > 2 )
    {
				...
				for(int64 j = igroup-1 ; j >= (StartFofId + 1) ;j--) {
					...
					for(int64 k=0;k<group[j].N;k++) {
						...
						if(octant >=0 && octant < NWEDGES) {
							...
							fprintf(stderr,"Error in calculating octant: I have octant = %d and it should have been (0 <= octant < %d) ",octant,
											NWEDGES);
							exit(EXIT_FAILURE);
						}
      ...
      while(flag == 1 && iter < MAXITER) {
				for(int64 igroup=StartFofId+2 ; igroup < (StartFofId + Nsub) ; igroup++) {
					/* 			  fprintf(stderr,"igroup = %"STR_FMT"  startfofid = %"STR_FMT"  Nsub = %"STR_FMT" \n",igroup,StartFofId,Nsub); */
					index = smallest_match[igroup-StartFofId];
					if(index != -1) {
						if(group[index].ParentLevel != -1) { /* Check if the container has ParentLevel already set (this should be the case, almost always) */
      ...
      fprintf(stderr,"This should not have happened -- trying to match subhalos on a NULL or only FOF group\n");
      if(group!=NULL)
				fprintf(stderr,"StartFofId = %"STR_FMT" and Nsub = %"STR_FMT" isfof = %3d ..exiting\n", StartFofId,group[StartFofId].Nsub,group[StartFofId].isFof);
...
void print_subhalolevel_header(FILE *fp)
{
  ...
  fprintf(fp,"##############################################################################\n");
  fprintf(fp,"#   FofNum          GroupNum  ParentLev   ContainerNum     Nsub  N_per_wedge #\n");
  fprintf(fp,"##############################################################################\n");
		...
		if(fp == NULL) {
			...
			while(FofId < Ngroups) {
				/*if the Fof has exactly one Nsub, i.e., the Fof halo itself, then
					we know the ParentLevel and the ContainerIndex (set in loadgroups.c) */
				...
				/* 	//Nsub == 1 */
				/* 	fprintf(stderr,"FofId = %"STR_FMT" Nsub = %"STR_FMT" Parentlevel = %hd \n",FofId,group[FofId].Nsub,group[FofId].ParentLevel); */
				/* 	fprintf(stderr,"WARNING: Hardwiring values - this should not happen - check the data load\n"); */
				...
				for(i=FofId; i < (FofId+group[FofId].Nsub);i++)
					fprintf(fp,"%10"STR_FMT"     %10"STR_FMT"   %6hd    %10"STR_FMT"    %10"STR_FMT"   %5d \n",
									FofId,i,group[i].ParentLevel,group[i].ContainerIndex,group[i].Nsub,group[i].N_per_wedge);
				...
				if(fgets(str_line, MAXLINESIZE,fp)!=NULL) {
					if(str_line[0] !=comment) {
						...
						int nread = sscanf(str_line,"%*d   %"STR_FMT" %hd   %"STR_FMT"  %"STR_FMT"     %hd", 
															 &dummy,&group[i].ParentLevel,&group[i].ContainerIndex,&group[i].Nsub,&group[i].N_per_wedge); /*discarding the first field, Fofid */
						assert(nread == 5 && "ERROR: Could not read in the 5 values expected from subhalo hierarchy file");
						if(dummy != i) {
