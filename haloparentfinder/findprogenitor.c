...
void find_progenitor(struct group_data *nextgroup, int64 NextNsub, struct group_data *prevgroup, int64 PrevNsub,
                     const char *outpath)
{
            ...
            while (found == 0 && best_match != -1)
            {
                ...
                if (old_parentid < 0)
                {
                    ...
                    fprintf(stderr, "A side-effect of using some fixed number of most "
                                    "bound particles ..\n");
                    fprintf(stderr,
                            "igroup = %" STR_FMT " parentid = %" STR_FMT " best_match = %" STR_FMT
                            " old_parentid = %" STR_FMT " \n",
                    ...
                    prevgroup[best_match].Switched == 0)
                {
                    ...
                    */
                    ...
                    fprintf(stderr,
                            " with old_parentid = %" STR_FMT " at snapshot = %d. Parentid for old_parentid = %" STR_FMT
                            "  \n",
                    ...
                    fprintf(fp,
                            " %12" STR_FMT " %12" STR_FMT "  %14d    %14" STR_FMT "  %12" STR_FMT "  %12" STR_FMT
                            "   \n",
                            best_match, prevgroup[best_match].N, nextgroup->snapshot, igroup, nextgroup[igroup].N,
                            PrevNcommon[best_match]);
